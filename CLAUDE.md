...
unlock/yield/lock round trip. Its entry is `OSAPI_FULLSCREEN` first (the
§11.2 surface, so the geometry lands in the WINDOW RECORD and the app's own
`OSAPI_WM_GEOM` call is the whole of its fullscreen layout) and then the
bracket; inside it draws its own XOR crosshair and polls its own input into
its own `W_ONCLICK`/`W_ONKEY`. `tests/fsxtest`
