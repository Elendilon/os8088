...
| **[docs/plans/REGION-SELF-COMPACT-PLAN.md](docs/plans/REGION-SELF-COMPACT-PLAN.md)** | a package that asks for the biggest claim it can get and does not get it. **DESIGN, not started.** SPEC.md 66.6.1 built everything a REGION needs to move and left out the one moment a package ever wants it: `mem_frameless` asks `mem_in_nest`, and **a package reaches `mem_claim` only from inside its own callback**, so `wm_pkgs` names its segment and the compactor pins the asker's own region **by the act of asking**. The report it came from is a DOS runner that unmounts the sound driver and cannot compact into the 14KB it freed; the geometry is MEASURED here and it is ordinary - a region is claimed TOP-DOWN, so the package launched first sits at the ceiling and the next one lands under it (Calculator at `9E000..A0000`, Browser at `99000..9E000` with `rloc=160`, so the DECLARATION is not what is missing). **Option 1 - patch the caller's return frame - is REFUSED and the reason is worth having**: the two words the KERNEL pushed are at computable offsets and are not the only two copies, because the package may have pushed its own DS (Paint does, at twelve sites) or be holding it in a register, so the rest rests on a declaration nothing can check whose violation is not a crash but a WRONG ANSWER. **What it proposes is two patterns this tree already ships, composed**: `OSAPI_PKG_REHOME`'s *"it only RECORDS: you are still executing in the region this frees, so the kernel does the work after you return"* (SPEC.md 20.12.10), serviced where `[ui_rebootq]` is serviced - `ui_task`'s step 0, *"a posted restart, spent with NOTHING held"*. `mem_can_move` is then UNTOUCHED: at that point the region is genuinely frameless and the existing predicate answers yes because it is TRUE. **Its 5.4 is the half nobody would have costed and the one that actually moves the number**: SPEC.md 66.4.1's two passes are ALTERNATIVES, and MEASURED on that machine the ascending one wins by 68.5K - so the answer is **494.5K whether the region is pinned or not**, and UN-PINNING THE CALLER'S REGION BUYS EXACTLY ZERO ON ITS OWN, by either option. Ceiling pass first and then the floor re-planned against it is 502.5K, the 8KB hole to the byte, and it is ~20 bytes - so the two halves land together or neither does. 5.5 is why `mem_avail` then needs NO change, 7 the four things this does not solve |
