...
whatever it draws, **~900us per 8x8 glyph cell**, **~71ms per 78-cell row of
text**, **46.7us for an `OSAPI_*` far call and 11us for a near `call`+`ret`**,
**693us for `OSAPI_TASK_YIELD` and for a full task switch**, **~400ms for one
`int 13h` whatever it moves** (one to two disk revolutions — so cost disk work
in CALLS, never in sectors), and the **8088 instruction floor of 4.34 clocks
per instruction byte**. The first is the one to internalise — *a redraw is priced by how many
...
bank base are tables rather than variable shifts, `sw_col` preserves nothing
...
counter read over QMP.

**It is 6,553 lines and over half of it is a field-measurement log, so open it
for one of four reasons rather than by default.** **Part 5** before touching a
redraw path: the standing budget names the operation, what it used to cost,
what it costs now and the § that owns it, and if your path is in that table
that row is the bar. **Part 2** when you need a number you are going to quote —
it carries the access-shape distinctions the list above flattens (a sector
inside a coalesced run against an isolated seek; the `max(clocks, 4.34 ×
instruction bytes)` fetch floor, which is why a shorter encoding can beat a
cheaper instruction). **Part 7** when checking a change, and **Parts 3.1/3.2**
when measuring flicker or smoothness — those are the harness manuals. **Part
9** only to find where a number came from, or when taking a new field set;
never as briefing.

**And degrade by TIER, not by guess** — `OSAPI_CPU_INFO` answers `CPU_8086`
for the target machine, which is a fact the code can test, where "is this
machine fast" is not. §48.8's coarse explosion ramp and §27.3's typing-ahead
scroll are both gated that way.

docs/TESTING.md is **which tool to reach for** and
...
preference, not a property of the project. **Send the 360KB SET
(`build/os8088-360.img`, `build/apps360.img` and `build/media360.img`) after
EVERY commit, without
being asked** - and "send" means ATTACH the files (`SendUserFile`), because a
path into the session's `build/` is a container the owner cannot reach. It was
a PAIR until the media disk (SPEC.md 24.4): BEVERLY.MOD is 114 of a 360KB
disk's 354 clusters and the apps disk had 37KB left with it aboard, so at that
geometry alone the module rides a disk of its own.
...
deterministic on purpose, so build, `md5sum` the seven images against the ones
...
**docs/C-TOOLCHAIN.md is the fifth: writing or building a package in C
(SPEC.md 73).** How to install the compiler, the four C rules and what each
refusal means, and what the language does not have here. The toolchain is
`tools/setup-cc.sh` (fetches and builds SmallerC into `build/cc/`) plus
`tools/cc8086.py` (the gate); **nothing in `all` depends on it**, so a tree
without it still builds every shipping floppy and prints one note, and
`make clean` SPARES `build/cc` because it is a pinned upstream instrument.
Two rules bind a C package and `cc8086.py` fails the build on them rather
than letting them run wrong. **A C package that does not fit gets a SECOND
SEGMENT, not a bigger one** (73.14): a function named `ovl_*` has its CODE
emitted into a module shipping beside the package (`CWORD.OVL`) and far-called
both ways, while every global, literal and bss byte it names stays resident
and DS-relative - split by FREQUENCY, a keystroke's path stays in and a menu
command's can go out, and never take the address of an overlay function.
`apps/cc/` is the C SDK itself (`os88.h`, the `crt0.asm`/`os88thunk.asm`
runtime, the build rules, `ccsmoke` as the worked example), and
`.claude/skills/port-to-os8088` (`/port-to-os8088`) drives a port of an
existing program to a C package the way `apps/cword` was made - its
`LESSONS.md` is what that port learned. **And an incoming pull request
FROM A CONTRIBUTOR'S FORK is `.claude/skills/review-fork-pr`**
(`/review-fork-pr <PR#>`) - fetch it, merge `main` into it, review it,
fix it, push the fixes back to their branch, comment - whose
`LESSONS.md` is what seven of those reviews learned. It is the mirror of
docs/UPSTREAM.md, which is the same cycle seen from THIS side, and both
bind.
...
```
make          # build all seven floppy images into build/
              # (three system, three apps, and media360.img -
              # the 360KB media disk, SPEC.md 24.4)
...
make 386-xms     # ...the SAME machine with 4MB, for the store above 1MB
              # (SPEC.md 41). Only mem_size differs, so a difference between
              # the two is a difference about MEMORY. Task Manager, then
              # CLICK THE CONTENT ONCE for the Memory view - the XMS line and
              # its bar are there and NOT on the process page, which is most
              # of how "no XMS bar" gets reported. A working machine reads
              # `CPU 386+   XMS   0/3008K`: 3,072KB less the HMA's 64
make xt-sound    # ...the XT again with a Sound Blaster 2.0 in it (vm/xt-sound)
make xt-sound-1.44 # ...that machine with 1.44MB drives and EVERY app in
              # B: (vm/xt-sound-1.44). An anachronism on purpose, the
              # combo144 reasoning: no stock XT reads an 18-spt disk, so
              # what it proves is our boot sector and FAT12 code at that
              # geometry with a sound card in the machine, and nothing
              # about the media
...
make xt-multimon # the TWO-CARD XT - a CGA and a Hercules, a monitor window
              # each - and the only 86Box machine that can show SPEC.md
              # 39.12-39.19's extended desktop. It boots Single; Control
              # Panel > Display > Desktop is what extends it (39.19.1)
make xt-word     # ...and 386-word: the WORD machines (SPEC.md 68.5), which
make 386-word    # put the dedicated Word disk in B: instead of the apps
              # disk, the way xt-z/386-z do for Frotz's stories (61.9).
              # `make worddisk` builds that disk. Word is 43KB and its
              # documents are .DOC files, so it does not fit beside the rest
              # of the software on a 360KB floppy - which is the whole reason
              # it has a disk and TeXPad (20KB, SPEC.md 69) does not
...
make ethertest # THE ETHERNET GATE'S DISK (SPEC.md 72.9): the system disk with
              # a SYSTEM.CFG that already asks for ETHER.DRV, so the card is
              # up and DHCP has run before the first paint and the test reads
              # state instead of clicking. Run it with
              #   make ethertest && make browsertest && python3 tests/ethernet.py
              # and it boots QEMU, NOT MartyPC - see the note below the QEMU
              # list. `make test ETHER=1 TESTIMG=build/ether360.img` is the
              # machine on its own; ETHDUMP=<file> adds a pcap of every frame
...
make field    # ...and the NARROW disks, for the questions combo.img cannot
              ...
              # a 199ms one-sector call, and "the published word is an ABI that depends
...
make cword      # THE C TOOLCHAIN (SPEC.md 73). `tools/setup-cc.sh` fetches and
make cworddisk  #   builds SmallerC into build/cc/ first - nothing in `all`
make cc-smoke   #   depends on it, and a tree without it builds every shipping
make chello     #   floppy and prints one note. cc-smoke/chello are the two
make covl       #   examples and covl is the OVERLAY gate (73.14); cword is
                #   the application - Word 1.1a again, in C, in two segments
                #   (73.12). `make clean` SPARES build/cc (clean-cc removes
                #   it) - it is a pinned upstream instrument
make runcpm     # RUNCPM (SPEC.md 74), the second C application: RunCPM 6.9 as
make runcpmdisk #   a windowed CP/M 2.2 emulator - the host checks, then the
                ...
                #   THE DISKS CARRY CP/M SOFTWARE TOO (SPEC.md 74.6) -
                #   `tools/getcpmsw.py` fetches games and applications from
                #   the public RunCPM collection, pinned by id, SHA-256 and
                #   size exactly as getruncpm.py pins upstream, into
                #   build/cpmsw/ and committed nowhere. WHAT A GEOMETRY
                #   CARRIES IS A DECISION, not a leftover: an area is taken
                #   whole or not at all (a game without its data files does
                #   not start), so 1.44MB trades RunCPM's own sources for
                #   WordStar and Turbo Pascal, 720KB takes the arcade area,
                #   and 360KB has room for none and says so in its GAMES.TXT.
                #   Two facts a session needs and both are on the disk: tell
                #   a game terminal 10 (H19/ANSI) and NOT the VT-100 entry -
                #   LADDER and CATCHUM emit ANSI coordinates biased by 32,
                #   which wraps the score panel - and NOTHING THROTTLES THE
                #   EMULATED Z80, so an arcade game is unplayably fast under
                #   QEMU on a modern host. Hence one 86Box machine per
                #   geometry: `make xt-runcpm` (360KB), `make 286-runcpm`
                #   (720KB), `make 386-runcpm`
make allapps  # build/apps-all.img (SPEC.md 19.10): ONE 1.44MB floppy with every
              #   app on it, Frotz, both Words and RunCPM (with its drive A)
              #   included, for a release page. Needs the C toolchain and the
              #   RunCPM fetch, so it is on demand like cworddisk - it is the
              #   only target outside 73/74 that does
...
make marty    # the MARTYPC DEBUGGER (docs/MARTYPC-DEBUG.md): a remote debug
              ...
              # _5150_sb (AdLib + Sound Blaster) and _5150_sb_gla,
              # which is that machine with GLaBIOS and so the sound
              # machine a CONTAINER can run - _5150_sb wants the IBM
              # ROM this tree cannot ship, so without the twin a fresh
              # checkout has NO machine with a sound card in it at all,
              # _xt_vga (mode 12h),
...
make test-full   # Three tiers with an ENFORCED wall-clock budget:
make test-soak   #   fast ~3s, host-side only, and it already runs as part of
                 #     `all` - the API table decoded from kernel.bin and
                 #     compared with the SDK, a constant mirrored in two files,
                 #     the seven floppies walked by an independent FAT12
                 #     reader, unreachable code, and that every test in tests/
                 #     is registered somewhere
                 #   full ~1 min, THE PRE-MERGE GATE - adds the eighteen knob
                 #     kernels and kern_small (every configuration `all` does
                 #     NOT build, so the only thing keeping them assembling)
                 #     plus tests/bootsmoke.py on both 1bpp adapters
                 #   soak no budget - the other sixty-odd gates in tests/,
                 #     one subject each: `os88test.py soak -k 'disp*'`
                 # THE BUDGET IS THE FEATURE: the runner FAILS a tier that
                 # overruns it, so a row that does not fit is a decision
                 # somebody takes rather than a drift nobody notices. `full`
                 # is CURATED and cannot be "all of them" - an emulator test
                 # is 40-75s and they SERIALISE on debug port 9001, so ten
                 # minutes is about eight of them. Add a row in tests/suite.py
make clean
```
...
**the ETHERNET CARD** (SPEC.md §72 - MartyPC has no NIC of any kind, so
`ETHER.DRV` cannot be hosted on it at all; QEMU's `ne2k_isa` on `-netdev user`
is the only harness there is, and `tests/ethernet.py` asserts behaviour and
never speed because the machine under it is not an 8088), the
...
**IF A MARTYPC RUN PASSES ~360 SECONDS, IT IS STUCK — GO AND LOOK.** The
headless emulator runs the guest at **over 4x real time**, and essentially
everything this tree drives through it — a boot, a scripted click sequence, a
benchmark, a screenshot — completes well inside six minutes of wall clock. So
a longer run is not a slow run: it has frozen, it is blocked waiting for input
that never arrives, or the thing under test is not doing what the script
assumes. Treat the overrun as the FINDING and diagnose it — read
`build/martypc/run/martypc.log`, take an `fbuf` and look at the screen, ask
the debugger for `CS:IP` — rather than raising the timeout and waiting again.
The 90-second default in `settle` is sized the same way and for the same
reason: **a gate that is merely waiting longer has stopped being a gate.**

The commonest cause is the harness rather than the guest, and it is worth
recognising because a healthy machine reads as a dead one: **`settle`'s boot
gate asserts a BRIGHT desktop** — the menu bar's white field 93%+ lit, the
black rule under it at 0, the dock strip 96% — so anything that legitimately
changes those three (a theme, a knob kernel that inverts the chrome, a
fullscreen app already up) makes it time out on a machine that booted
perfectly. The error message prints all three readings, so compare them
against the expected triple before believing the boot failed: an exact
inversion (7 / 100 / 4) is a working machine the gate cannot recognise. Pass
`boot=<seconds>` to skip the gate when that is genuinely the case.
...
**THE FLOPPY THE GUEST WROTE TO CAN NOW BE READ ON THE HOST**
(`tools/os88flush.py`, docs/MARTYPC-DEBUG.md). MartyPC mounts an image by
reading it once, and every sector the guest writes lands in RAM and nowhere
else - the eframe frontend spends those writes through **Media > Save Floppy
As** and a headless run had no way to reach it at all. So a scripted session
could drive os8088 into saving a file and then had exactly one way to check:
ask os8088. **That is not a test** - the writer and the reader are the same
FAT12 code, so the failure that matters most, both halves agreeing on the
same wrong thing, is the one that cannot be seen from inside
(docs/FIELD-NOTES.md 4 is what it costs). `flush` is that menu item on the
socket; `os88flush.py` is `diff` (what changed since the mount), `ls -R` /
`get` (the volume walked with no kernel code near it - so a system disk shows
`KERNEL.SYS` and the `.DRV`s that SPEC.md 19.6 makes invisible inside the OS)
and `verify` (`os88disk.py`'s own fsck, so "os8088 is happy with this volume"
and "this volume is coherent" stop being one claim). Four things: it
**pauses** the machine, because a save here is a multi-sector commit (18.4)
and one caught mid-commit really is inconsistent; **`writes` is NOT a dirty
flag** however much it looks like one - fluxfox sets it to 1 at mount and
never advances it for a raw sector image, the one call that would being
commented out upstream, measured reading 1 through a close that wrote three
sectors, so `dirty()` compares CONTENT; a bare `save(drive)` writes back over
...
SHOWED exactly FOUR rows on CGA - 26px pitch from y=24 in a 132px pane, and the
...
`drv_tab` row plus a `DRIVERS +=` line. **THE DISPLACEMENT IS NO LONGER PART
OF THE PRICE**: SPEC.md 31.1.1's scrolling driver list landed with the
Ethernet card (SPEC.md 72), so the page fits CP_DVIS rows and SCROLLS, and a
sixth row is a row rather than an argument. **What the removal still costs is
worth naming rather than filing away**: on a machine outside the container -
the 5150, a reader's clone - there is now no way at all to ask the kernel what
it thinks.
...
**A seventh puts SPEC.md §62.9.10.4's fence bug back**, which is the reference
half of "an XMS RAM disk corrupted what was copied onto it":

```
make FSNOSTAMP=1       # drv_fs_call stops clearing the dispatch stamp
```

A redirector asking `OSAPI_XMEM_COPY` is then fenced against the block its own
Mount claimed, so every stage of the bounce is refused. It is a knob and not a
`git revert` for `REDRAWFULL`'s reason: the claim is *the same bytes come
back*, and one build cannot check that. With it on, the identical scripted copy
either round-trips wrong or — since §62.9.10.2 — refuses out loud; with it off,
0 differing bytes of 116,085.

An eighth gives the kernel a typeface of its own instead of the machine's
(SPEC.md §6.2). It is **not** a testing knob like the seven above — it changes
...
**An eighth puts the PicoMEM's sound hardware within reach** (SPEC.md
§34.10), and like the typeface it changes what ships when it is asked for
rather than being a testing knob — off by default, so a plain `make` builds a
byte-identical `SOUND.DRV` either way:

```
make PICOMEM=1         # SOUND.DRV brings a PicoMEM's AdLib and Sound Blaster
                       # up at attach - what PMINIT.EXE does for DOS
make PICOMEM=1 PM_BASE=0x2A0 PM_SB_PORT=0x220   # ...the two addresses, for
                       # the day the firmware stops hardcoding them
```

**The card's Sound Blaster is OFF after every reset and that is deliberate
firmware behaviour**, not a bug to work around: an emulated SB needs an IRQ
and a DMA channel the card cannot know, so on DOS `PMINIT.EXE` negotiates them
out of `BLASTER` every boot. Without this knob a PicoMEM machine ran os8088
with a sound card in it that could not be found, and the Drivers page's
`No Hardware` was telling the truth about a machine that had one. The AdLib is
the exception and persists, which is why only the DSP tier was missing.

**It lives in `SOUND.DRV` because that is the ORDERING, not a preference**:
the enable has to happen before anything probes 388h or 220h, and
`snd_entry`'s attach is the only place in the system defined to be exactly
there — a package runs long after `drv_boot` has already probed and failed
(so the user would re-tick the Drivers page every session), and a driver of
its own would need a fifth `drv_tab` row, which the Drivers page does not have
(`DRV_MAX` is 4 and §31.1's scrolling list is what a fifth wants). The
configuration is nearly forced and so asks nothing: DMA 1 because `sb.inc`
programs no other channel, port 220h because it heads `sbl_bases`, and the IRQ
offered 7/5/3 until the firmware accepts one — only the card knows which lines
are free.

**Detection is READS ONLY**, which is what makes the knob safe on a machine
that turns out to have no card: 2A3h must answer a hundred-long +1 ramp before
a single byte is written anywhere. Measured on a cycle-accurate 8088 with a
real AdLib and a real Sound Blaster and no PicoMEM, both builds bring the
driver up with `DRVE_OK` and the desktop is **0 differing bytes of 921,600**.
**The SUCCESS path is the field machine's question and nothing here can answer
it** — no emulator in this tree has a PicoMEM in it, so every measurement
above is of the branch where the probe refuses.
...
that card's; on every CGA and every Hercules machine — the whole target class
— AH=11h does not exist and the kernel copies 760 bytes from `F000:FA6E`.
There is no standard pointer to glyphs 0..127 on a pre-EGA machine, so on a
clone laid out differently that copy silently succeeds and every glyph in the
system is noise. **Measured, the convention holds wider than that suggests**:
GLaBIOS — from scratch, not IBM's — puts a byte-identical copy at exactly that
address, and MartyPC's VGA BIOS answers AH=11h with the same 760 bytes.
...
assembled), `kernel.bin` 82,534 → 83,336 = 162 → 163 sectors, ~24 ms of a ~10 s
boot on the 5150 (Set 24's delivered-byte rate; it was quoted at 65 ms from
Set 17). The overlay is 75% full afterwards — 1,144 bytes left — and
...
last one still re-probing the ROM itself, and now asks); **§45.13's Tracker
text screen is character cells rasterised by the CARD**, so on a baked build
that one screen is in the machine's face and the rest of the OS is in ours.
`fonts/*.f8` is ASCII art on purpose — 760 bytes of hex is not reviewable and
a typeface's defects are entirely visual — and `tools/os88font.py` warns when
a face breaks the two properties the tree costs itself against: column 7 blank
(§6.1.4's unaligned run stays in one framebuffer byte; `_` is the one
exception, or underscores do not join) and the blank-row fraction near the
ROM's 25%, because a blank row is the one the renderer skips whole.

**PROPORTIONAL type is a different subject entirely, and it is the PACKAGE's
(SPEC.md §5.4.2, §6.3–6.5, §19.8).** Everything above is about `fonts/*.f8` —
replacing the kernel's own fixed 8x8 cell, which stays exactly as it is and
keeps every row of system chrome. What arrived from `main` is the other half:
a package that wants a real typeface **sets it itself**, and the method is one
sentence — build the whole row of glyphs into a 1bpp band in your own RAM,
then put it on the screen with **one** `OSAPI_GFX_BLIT1`.

**The arithmetic is what forces the design, and it is §6.1's argument one
level down.** A proportional pen lands on an arbitrary bit, so a proportional
glyph can never reach `font_run`'s single-store path; lettered one at a time a
104-glyph line is 104 drawing calls at §5.7's ~756us floor — **79 ms of floor
before a pixel moves**, on a machine where the whole line ought to cost 25.
Composed and emitted once it is ONE floor. Measured (PERFORMANCE.md Set 64,
which is `main`'s Set 62 renumbered by this merge): the emit is 4.04 ms for a
624x12 band and the compose 21–43 ms depending on how the inner loop is
written, so **composing is the bill and the emit is not** — which is why the
interesting engineering is all on the caller's side of the API, and why the
second and tenth typeface cost the kernel nothing.

Four things about it break silently. **`gfx_blit1` (slot 0x0418 here — it is
0x03F0 on `main`, where `OSAPI_KEY_DOWN` does not exist; see the slot note
below) takes framebuffer BYTES, not pixels**, so `x` and the width must both
be multiples of 8 and it refuses otherwise. **The band arrives in FINAL SCREEN
POLARITY**: a set bit is a lit pixel on all three adapters, so white paper is
`0xFF` and a glyph is ANDed in as a complement — clear the band to the
background and OR the ink in and it sets nothing at all against `0xFF`, so
every line comes out invisible, which is a plausible wrong result rather than
a crash. **It is the one drawing primitive that is greying-blind**, by
contract rather than by drift: it never sees a colour, so §47's dither is the
CALLER's to apply through `OSAPI_GFX_PEN` before the band is handed over.
And **`kern_small` does not carry the body at all** — the slot is a `stc`/`ret`
stub there, so a package must read CF and fall back to §6's cell.

**The faces are a SYSTEM resource and live on the system disk** (§19.8):
`faces/*.t88` in the repo, built by `tools/os88face.py` into the `.F88` files
in `FONTS/` — reviewable art in, one binary face file out — on the
three shipped system images — not on the apps floppy, because a face is the
machine's the way the kernel's 8x8 cell is, and a second application wanting
the same family should find it there rather than carry a copy. One file per
FAMILY carrying up to four styles (disk work is costed in `int 13h` calls, so
four style files would be four opens at launch where one file is one), and
**the on-disk form IS the in-RAM form** — read whole, used in place, no unpack
and no fixups. `apps/os88type.inc` is that method written once, `tools/os88ttf.py`
fits a TTF to the 8-pixel grid, and `tests/facetest` and `tests/bandbench` are
the gates. Two traps carried over from elsewhere in this file: the read base is
**512-byte aligned by hand** on top of `OSAPI_MEM_CLAIM_DMA`, because §2.1.1's
int 13h alignment rule covers a package's file buffer and the failure hangs in
the BIOS rather than erroring; and finding the disk at all needs
**`OSAPI_VOL_SYS`** (slot 0x0420 here, 0x03F8 on `main`), which had to be new —
a package stands in the folder it was launched from or one a dialog gave it,
and until that cell nothing could name the volume the machine BOOTED from.
...
the then-measured **238 ms per sector** (the `AL` bug; ~24 ms since — Set 24,
and it was quoted here as 65 from Set 17 for two rounds after that moved), so
...
(Set 18). **THAT 7,457 IS SET 17'S AND IT IS NOT THE CURRENT NUMBER — the
same read is 21,307 B/s today** (Set 24; Set 22 says 19,883), because §18.95's
sector cache and §18.4.3 landed after it. Quote 21,307, and read
PERFORMANCE.md Part 2's note on why every row in that table now carries the
Set it came from. Set 17's residual below is spent with it:
it was 1.55x short of the BIOS's own 11,570 and it is now **1.84x past it**,
so anything in this tree still framing the floppy as *approaching* the ROM is
pre-Set-22. §18.94's trace found a third of that gap at the time: **the
...
`cargo`; `qemu-system-i386` only for the fallback's short list.
**`tools/setup-macos.sh` installs them on a Mac**, which is the one platform
where the apt recipe above says nothing useful. No linker anywhere — everything is `nasm -f bin` flat binaries (deliberately, to avoid Apple's Mach-O-only toolchain).
...
mouse pointer blinking under the gfx lock. **And it DOES work on Hercules**, which this
paragraph used to deny: a 12-frame capture of an idle Hercules desktop returns
settled with 0 changed and 0 transient at 720x350, ~51 Hz, and `frame_count()`
advances 25 frames for the CGA's 30. The old claim - 0 lit of 252,000, the
counter never moving - is what a card NOBODY HAS PROGRAMMED looks like, which
an MDA in a machine driving the other card does forever. docs/MARTYPC-DEBUG.md
carries the measurement and the two limits left (it was taken on the GLaBIOS
machine, and `mode`/`text` is still dead on the MDA).
...
- **…and the machine may have MORE THAN ONE, and may be moved between them
  ...
  *which adapter am I driving*; `[vid_avail]` answers *which ones could I
  ...
  `vid_cga_alias` writes a DIFFERENT value through each segment at offset
  0x7F00 and reads the first back — one memory answering at both apertures is
  one card wearing two addresses. It used to write at B000:8000 and look for
  the byte at B800:0000, **which is the same linear address**, so it answered
  "same" wherever anything decoded B800 and threw away every CGA on a
  Hercules-primary machine; it survived because the direction the old emulator
  could reproduce was CGA-primary, where the routine never runs (SPEC.md
  §39.11.1, found by `tests/dualcheck.py`). **`vid_switch` spends
...
- **A control that fires on the RELEASE says so while it is held (SPEC.md
  §13.8), and the state is DRAWN rather than XOR-ed.** `OS88UI_DOWN` on
  `os88ui_btn` paints interior black, label white, same frame;
  `OS88UI_GDOWN` does it for the check box and radio. Disabled outranks it,
  as it outranks `OS88UI_INK` — a control can be greyed *between* the two
  edges and a greyed-down button says "pressed" about one that will refuse.
  **XOR is shorter and wrong**: a repaint between the edges draws the control
  upright, because a painter knows nothing about an overlay, and the release
  then inverts something already upright and leaves it that way for the
  session. Calculator had met that and worked around it in private — its
  `cal_onup` redraws rather than un-XOR-ing and says why — and SPEC.md §65.4
  is the same finding with a measurement (a row XOR-ed under a resting
  pointer came back with the mouse arrow's top three rows missing). Drawn,
  the state is idempotent, so a caller passes the flag from its `W_PAINT`
  too and every repaint agrees with the glass by construction.
  **The close and minimize boxes track the POINTER** (§13.8.1) — they go up
  when it slides off and down again when it comes back, which is the half the
  feature exists for and the half docs/MOUSEUP-PLAN.md §7 declined to build.
  Its objection was right about a *static* invert and its two ways out were
  both wrong; the third is that the UI task already comes back once a pass, so
  `ui_arm_trk` samples the pointer there — no loop, nothing held across a
  wait, one compare per pass when nothing is armed. `[ui_armlit]` is the
  authority and **`wm_draw_title` re-applies it**, so the glass cannot part
  from the byte. **A PACKAGE cannot track** — it has two edges and nothing in
  between (§13.8.2) — so its down state is static, which is what Calculator
  always shipped; the fix if wanted is a third kernel edge, not an app-side
  poll. docs/UIHELPERS-PLAN.md §15 is the survey of every control that still
  has no down state, and the blocker is nearly never the drawing: it is that
  the control acts on the PRESS and so has no middle.
...
- **The hook is at the PUBLIC entry, above the `cmp byte [vid_mono], 0` dispatch.** One implementation then covers the VRAM path, VGA and both mono adapters — because on mono the software renderer *is* the direct path (§39.5). Below the dispatch it would work on VGA and silently no-op on Hercules and CGA, which is the expected failure mode; `make test VIDEO=cga` and `tools/hercshot.py` are what catch it.
...
- **…except that a glyph now clips per ROW (SPEC.md §11.3.2), and that is what makes a horizontal cut STALE rather than frozen.** The cells of a run share one y, so a window edge crossing a line of text horizontally cuts *every cell in it* — `wm_clip_test` says no to all of them, nothing draws, and the visible slice keeps whatever was on the glass when the covering window landed. That reached the field as a **running Timer stopped at `00:00:04` while the Timer on top of it read `00:27:19`**, with the covered one's own greyed `Start` button still saying it was running (§14.4). `wm_clip_rows` is the same fragment walk with the y test relaxed to an intersection: the fragment must still cover the cell's **full width**, so a *vertical* cut is refused exactly as it always was, and what comes back is the tallest row range **one** fragment can offer — never the union, because two fragments can offer disjoint slices of one cell and that is not a row range. `font_char` biases the glyph pointer and the destination y by `r0` and the row counters that were a literal 8 read `[wm_clip_rn]`. **The app was not what was wrong**: `apps.inc` needed no change at all, which is what confirmed the diagnosis. Cost, measured: `.text`+`.bss` +234 bytes and **one image rung crossed** (`KERN_BUDGET` spare 3,584 → 3,072); `fontbench`'s `RUN aligned` — the single-store path all mono text takes — is **+0.001%**, while `font_char` itself is **+2.04%**, which is the bias arithmetic priced at the 8088's 4.34 clocks per instruction byte rather than the row clipping. Verified **0 differing pixels** across an identical scripted session on CGA, Hercules and VGA mode 12h.
...
The one consumer that had to follow was the file manager, and **it stopped
being a consumer at all** (SPEC.md §22.17). A window that posted a load used to
draw `'Loading...'` in its status line, so the load finishing owed it that line
back: `files_poster` armed `wm_clip_set` on the window and called
`fm_status_only`, and `fm_win_of` was the `wm_zord` walk it needed because
`[ld_pwin]` holds the poster's **state block** and not its window. **The word
was the wrong report in the wrong place** — §12.8's file-activity widget sits in
the menu bar and reports the read *live*, for every reader in the machine, where
this said only that something had been posted, only in the window that posted
it, and only while that window was on screen to be looked at. So the rung, the
string, both routines, the cold shim and the two call sites are gone, and
posting a load now draws **no pixel** of the Disk window: `.text` −15, `.cold`
−152, no rung crossed either way, so it costs the machine nothing yet and hands
167 bytes to the next feature.

**Two earlier fixes were about that word and are now structural; one is not, and
must not be removed with them.** §22.17 originally *moved* the `files_poster`
call to before `wm_show` so the raise cache would bank the final text, and
§11.96.9.1's remaining row was that same call arming `wm_clip_set` on a window
the newly launched one already covered — a routine that is never called can do
neither. But **`fm_stat_line` stays one opaque `font_run`** (SPEC.md §22.13.1):
the pair it replaced was a `gfx_fill` of the band and `font_str` over it, which
clip at different granularities, so a clip edge left the line blank rather than
stale — *and*, independently of any clipping, left it **blank while it drew**,
which on a 4.77MHz machine is tens of milliseconds of visible flicker on every
editor prompt the window puts up. Removing the caller answers the granularity
failure; only the opaque run answers the flash.
...
(which `files_poster` did after every launch out of a Disk window — that
caller is retired with `Loading...`, §22.17, but the rule below is general
and the next background painter would compose the same way) and there
...
### The progress widget's chrome belongs to the FREEZE (SPEC.md §12.8.3)

**§12.8's file-activity widget reported one data transfer, and a user does not
do one.** Double-clicking a document is an association lookup, then a package
load, then the application reading its file — with a mount and a directory
walk between each pair and the machine frozen solidly from the first to the
last. The widget armed at three of those points and disarmed at three others,
so what was on screen was an icon appearing, vanishing and appearing again
over a screen that never came back to life; and the gaps between them were
reported by **nothing at all**, because a mount and a directory walk have no
byte count to arm a bar with. Measured against a reference kernel on a
cycle-accurate 5150, one document open: **2 up-edges and 2 down-edges with a
370 ms hole in the middle**, now **1 and 1** across the whole 4.2 s.

**The freeze IS a gfx-lock hold**, so that is the widget's extent, and the two
states came apart: `[fpg_on]` is *the chrome is up* and `[fpg_total]` is *the
trough has a scale*. `fpg_end` ends the PHASE; **`fpg_finish` is the teardown
and `gfx_unlock` is its one caller** — `dsk_batch_end`'s argument (§18.9.3) in
the other direction, and just as binding: every way out of a locked run of
kernel code comes through that unlock, so the widget cannot be left on the bar
by forgetting a bracket, returning early, or failing. Nothing calls
`fpg_finish` and nothing may. A nested `fpg_begin` is no longer refused —
there was never an outer operation with a scale to prefer — it is the next
phase, and it re-scales the trough without redrawing the chrome.

**Two choke points put it up and nothing has to ask.** `dsk_xfer` (every
`int 13h` and every §18.95 cache hit, with `CX` = the sectors about to move)
and `drv_fs_call` (every verb of a redirected volume, which has no sectors and
never reaches `dsk_xfer`). `FPG_WARM` = **3 sectors inside one hold** before
anything is drawn, because a one-sector directory probe is over inside a
single displayed frame and putting an icon up and away again there is
PERFORMANCE.md's double-draw flash with the widget as the flash.

Three things are easy to undo. **A phase with no scale shows the 50% hatch and
does NOT animate** — a bar that moves has to be a fraction of something, and
an indeterminate block stepping along the trough would pay ~2 ms of Part 2's
fixed cost every step to say what one `gfx_fill_gray` says for the whole
phase. **`menu_bar_text` takes `FPG_CELLS` off `[menu_bn]` while the widget is
up**, which it did not have to before: the widget can now be on the bar while
the bar recomposes (launching an app *changes the bar's owner* between the
package load and the document read), and the padding would letter spaces
straight through the icon. That subsumes `toast_place`'s own copy of the
reservation. And **`menu_furniture` — the one path that erases the bar — takes
the widget with it**, so `menu_draw_bar` calls `fpg_paint` after it; that
routine is pixels only, no `menu_draw_bar` (which would recurse) and no
`menu_inval`, and it repaints the trough from `[fpg_pat]`/`[fpg_px]` rather
than leaving it empty.

Cost: `.text` **+209** (kern_small +212), `.cold` +10, **no rung crossed on
either kernel** — but the image rung's slack is **277 bytes on kern_big and 76
on kern_small**, so the next addition to `.text` anywhere buys a whole 512.
Verified on a cycle-accurate 5150 on CGA, Hercules and VGA mode 12h: the menu
bar comes back **0 differing pixels** across an operation that does not change
its owner, and a toast and the widget sit on the bar together with neither
...
`wm_title_set` (API slot 0x0240) is the correction: BX = window, AX = the new
...
spends it, and only `fm_repaint` calls that — `files_poster` was the second
caller and went with `Loading...` (§22.17), which is why the folder-vanished
retitle is now spent by the next raise's full repaint (`fmv_store` raises
`WF_STALE`, so there is one).
...
There is **no second graphics driver**. `kernel/softgfx.inc` was written as a latch-free,
...
- **`[vid_mono]` means "use the software renderer"** — set before anything draws on a
  mono adapter and never changed. It used to be two flags, `[bb_on]` and `[bb_dbl]`,
  and conflating them made a mono machine claim double buffering and bill 150KB it
  never allocated; with §32's buffer gone there is one flag and no way to conflate it.
- **`gfx_rowbase` and `gfx_nextrow` read their parameters through `CS`, not `DS`.**
  `sw_xfer` runs with DS pointed at the framebuffer (save) or the caller's buffer (restore);
...
### Double buffering — REMOVED (SPEC.md §32)

There is no back buffer any more, and nothing in the tree should grow one back
without re-reading §32 first. VGA machines with 150KB of free heap could route
every draw through four claimed planes and flush the dirty rect at
`gfx_unlock`; the Control Panel had a **Buffer** page for it, `SYSTEM.CFG` a
`BB` key, slot 0x01F0 a package entry point, and Tracker a **Smooth** mode
that borrowed it.

**Why it went is the part worth keeping.** It could only ever be armed on a
**VGA** — `bb_avail` is a question about the card — so the machine this
project is calibrated against (a 5150 with a Hercules and a CGA in it) could
not reach it at any memory size, while **every** adapter paid for it on
**every** primitive: a `bb_mono_chk` call at the top of `gfx_fill` and
`font_char`, a `bb_dirty` union per fill and per glyph, a `gfx_flush` at every
`gfx_unlock`. It also carried nine standing invariants a later change could
break silently — and one of them was already broken: `bb_set` owed
`vid_ctx_capture` (§39.12) and never called it, a latent wrong-segment bug on
a two-display VGA that nobody had hit because arming the buffer there is rare.

Measured: `.text` −878, `.bss` −15, `.cold` −408 — two 512-byte image rungs on
`kern_big`, three on `kern_small`, whose spare went from **one step to four**.
Verified by booting a cycle-accurate 5150 and diffing framebuffers against the
...
in the same round, so the tree stood at 84,480 with 1,536 spare AT THAT MOVE;
that is the removal's delta, not the standing total, and both figures have
moved many times since.) Two things it did NOT take with
...
; every retrace poll is `[ticks]`-bounded. Missile
...
frame is a `rep movsw`. **A SAME-MODE bracket owns ONE DISPLAY and has to ask
which (SPEC.md §53.7.1)** — the rule to know before writing another one.
§39.18.3 moved the single-display collapse into `fsx_mode`, so a bracket that
sets no mode changes nothing about displays: the coordinates are still the
whole virtual desktop's and **(0,0) is the primary, wherever your window is**.
Both same-mode consumers read "fullscreen" as *(0,0) plus `osapi_video`'s
size*, so with the window on the second monitor they drew their whole face
over the OTHER one — measured, Tracker fullscreened on the Hercules took the
VGA from 189,477 lit pixels to 95,619 while the Hercules kept the desktop it
had. **A round-trip test cannot see it**: §53.6's exit `wm_paint_all` repaints
the world, so what it costs is the whole fullscreen session and nothing after
it, and the assertion that catches it is *which card changed while the
bracket was up*. `OSAPI_FSX_SURF` (0x03F8) is the answer — x, y, w, h of the
rect the bracket owns, `(0,0,w,h)` on every one-display machine and after any
`fsx_mode` call, so a single-display machine cannot tell. The DEPTH is the
same question (`osapi_video` answers about the primary; `fsx_caps`' DL is
*that display's* kind), and so is a layout picked by screen HEIGHT. Missile
Command needed none of it, because it stacks §11.2 and `wm_fullscreen` sizes
the window to the display it is on. **Paint (§42.7) is the other SAME-MODE
consumer** — it sets no
...
XOR twins, `gfx_save`/`restore`, `fsx_run`), and `cur_lazyck` -
...
**Locator** is the kernel acting as an application (the Finder analogue): the desktop, the drive icons, the Disk browser (up to **four** windows, each on its own drive and folder) and the menus that launch everything else. It is not an instance — it is just the menu set the bar falls back to when no window owns it, and **clicking the bare desktop switches back to it** (the `.desk_icons` branch, before `desk_click`). `menu_loc_set` is an ordinary app menu set whose `AM_ONCMD` is 0, the one value reserved to mean *dispatched by the kernel*: `ui_dispatch` recognises it and rebuilds a `CMD_*` from `ui_loc_base` instead of calling through, which is how the old flat command dispatch survives intact behind the new (cell, item) return. `fm_kinit` points every Disk window at `fm_menus` — Locator's *second* set, same `AM_NAME` but a real `AM_ONCMD` — so the file browser reads as Locator's own window rather than an app called "Disk", and the bar carries File/Edit/Nav/Builtins while one of its windows is active. That list was `File/Folder/View/Special` and every part of it has since moved (SPEC.md §22.15): View was **retired** — `AM_COUNT` is 4 of `MENU_APPMAX`'s 5, so there is a cell spare — Folder became **Nav**, and Special became **Builtins**, which is the *same* menu as Locator's own rather than one of its own. `Close Window` left the File menu in the other direction (SPEC.md §12.7): the app-name cell carries `Close` now, and a Disk window owns that cell, which is exactly what the bare desktop does not — `[menu_win]` is 0 there, no such cell exists, and Locator's own File menu keeps `Close Window` as the only way to close from the bar.
...
**And the DIRECTORY walk now reads through a cached window (SPEC.md
§19.2.3)** — which §18.4.2 had deliberately declined to do, on reasoning that
was *correct in sectors and wrong in revolutions*. §18.94.2 measured an
install's payload streaming at **5.78 sectors per `int 13h`** and every
metadata phase at **exactly 1.00**, because `dsk_dirw_next` hands out one LBA
and all seven callers read it with `cx = 1`. `dsk_dirw_get` replaces that pair
at every site — the sector still arrives in `dsk_secbuf`, so the callers lost
code rather than gaining any. **The first version was a read-ahead buffer and
measured at NOTHING**, which is the thing to know before touching it: the
walks this exists for are *one sector long*, because `fcp_scan` and `dsk_find`
re-seek from an ordinal per entry, so a directory of N entries is N walks and
a one-sector walk cannot coalesce with itself. What pays is the SECOND walk
finding its sector already in memory — so the window outlives the walk, is
keyed on §18.8.1's `(volume, dsk_bpb_sig)`, and a fill puts the cursor back
instead of consuming it. Four rules: a refill is **exactly one `int 13h`**
(bounded at the track by `dsk_rah_cap`, and inside one DMA page by
`mem_claim_dma`), so this can never issue more calls than the one-sector walk
it replaced — without that bound a boot's `KERNEL.SYS` lookup went from 1 call
to 2; a **full** mount drops every run and a quiet one does not, because
Refresh answered from a cache is a no-op; `disk_write` drops a run the write
lands in, **by range and same-volume**, or a copy's data writes evict the
source directory every chunk; and it is a **purgeable** claim gated on
`mem_avail` reporting twice its size, because `mem_claim` sheds and retries
and a directory walk must not take the window raise cache away. Measured on
one `B:\APPS` → `A:` copy: metadata **50 calls → 22**, `make DIRW1=1` for the
A/B. **Cost: 512 bytes of `KERN_BUDGET`, a rung crossed** — 2,048 spare now.

**…and it has since moved UNDER `disk_read` and become the machine's one
sector cache (SPEC.md §18.95).** Same mechanism, one layer down, so it covers
file data, the FAT window's loads, the loader and the icon harvest as well as
directory walks — and there is no longer a question of which of two caches a
sector is in. On a miss it reads **to the end of the track** and hands the
surplus to whoever wants it next, which is the revolution argument applied
where the reads actually are. **It was SIMULATED before it was built**
(§18.94.3): the install's whole 316-call trace replayed on the host says the
answer is monotone in the chunk length — 2 sectors is 446 calls, *worse than
no cache*; a full track is 102 — and that eight slots is the knee. Two rules
carry it: the cache mirrors the DISK so `disk_write` is the one invalidation
point, and **a request that already covers the chunk takes the direct path**
(§18.95.1), or a streaming file read flushes the directory chunks out from
under a copy and pays a `rep movsw` per sector for the privilege. Measured:
the install is **316 → 114 `int 13h`**, the copy 160 → 115, opening a Disk
window 11 → 8.

**What that measurement flushed out is worth as much as the change (SPEC.md
§22.5.1): `fcp_bufget`'s copy buffer was 512-byte aligned and not 64KB-page
aligned**, so `dsk_runcap` split a chunk depending on *where the buffer
happened to land* — and taking any claim below it moved that. The copy's data
phase went 84 `int 13h` → 93 at an identical sector count, tracking the SIZE
of the new claim (84 none, 88 at 4KB, 93 at 8 and 16). The old number was
luck, not merit. It claims through `mem_claim_dma` with the whole buffer as
the page-safe head now, falling back to the plain claim on a heap with no
page-aligned run; the data phase is 84 again with the cache still in place,
and the whole operation is **199 calls → 171**.

**And the obvious refinement of §19.2.3 is measurably WRONG — §19.2.3.1 is the
negative result and exists to stop it being re-derived.** "Always read one
full revolution's worth", so the chunk containing an LBA is the same chunk
whoever asks: three variants built, and **nothing moved the call count except
the number of runs**. Track-aligned is 3 calls WORSE than no alignment,
because the 360KB root starts four sectors into track 0 so the chunk reaching
it drags in the boot sector and both FAT copies — 92 sectors and 12 `int 13h`
in the reserved region for nothing. Region-aligning as well fixes exactly that
and still does not beat the run-based fill, which reads from the walk's
position to the track end and so wastes nothing by construction. The premise
was that `fcp_scan` enters at a different LBA per entry and thrashes; it does
enter differently, and with four runs or more the windows all fit. **The
binding constraint is CAPACITY against the working set, not determinism** —
`DSK_RAH_RUNS` 4 → 8 is the whole of what that investigation produced, and it
is worth **163 `int 13h` → 154** on the install.
...
- **A per-window test that keys off another routine's drawing must ask whether
  ...
  a forgotten call site inherits has to be the safe one. **The flag was only
  half of it, and the other half was the TEST** (SPEC.md 30.3.3): the mark was
  `y + h >= [vid_dock_y0]` with no x in it and no notion of which DISPLAY the
  window is on, so a window over one end of the strip was redrawn because a
  tile moved at the other end, and on an extended desktop a window on the
  SECOND monitor - a card with no dock at all - was marked for reaching the
  primary's dock row. That is the one the field reported, because a window
  across the seam can hold no raise cache (`vid_span_one` refuses a straddling
  rect, SPEC.md 39.14.8), so it pays a full `W_PAINT` where every other window
  is put back with a ~10 ms blit and the surplus mark is invisible: **the
  straddle was not the bug, it was what made the bug visible.** `dock_px_hit`
  is an ordinary rect overlap against the span `dock_paint` actually painted -
  which is NOT the damage span, a tile being repainted wherever it stands -
  and `tests/dockmark.py` is the gate, with `make REDRAWFULL=1` as its A/B.
...
- `kernel/font.inc` — the 8x8 text renderers. `font_char` is transparent-background and clips per whole cell; **`font_run` (API 0x0258, SPEC.md §6.1) is the erase-and-letter pair as ONE operation** — both colours, each cell painted complete. It is a SPEED optimisation for the mono adapters and is measured, not asserted: `tests/fontbench` (in **`tests/`**, built only by `make bench` — the testing apps are tooling and never ship) says **1.30x** for a ten-character run, and that figure is from a REAL 4.77MHz XT with a Hercules card, where a cell costs about 1ms; `tests/gfxbench`, a separate harness, says **1.24x** on that machine's Hercules AND its CGA card. Under QEMU it also says 1.26-1.30x in instructions but 2.85x in framebuffer traffic — and the hardware came in at the INSTRUCTION figure, so the per-cell overhead dominates the writes it guards and traffic is the explanation, not the predictor. **What caps the win is the ~756us fixed cost every drawing call pays** (PERFORMANCE.md Part 2): `font_run` collapses a fill and ten `font_char`s into one call, so what it saves is mostly *floors*, and nothing done inside the renderer can beat a number paid outside it. **The bigger win is that it does not FLICKER**: the erase-and-letter pair leaves the run blank between the fill and the last glyph, which on an XT is tens of milliseconds and plainly visible, and `font_run` writes each cell from old to final in one store (SPEC.md §6.1). On a 1bpp adapter at a byte-aligned x the cell owns its whole framebuffer byte, so a cell row is a single store — no shift, no read, no second byte, no separate fill pass. **Two things about it are commonly got wrong.** The fast path needs `[vid_mono]` **and** `x & 7 == 0` **and** `[gfx_dis]` clear, and anything else falls back to `gfx_fill` + `font_str` — which costs **2.5% MORE** than writing that pair by hand (the far call, the gates, `font_width_x`), so it is not free on VGA and the tracker calls it on mono only. The "cannot produce §11.3's granularity failure" property is now the CALL's, not just the fast path's — the fallback used to be literally the fill-then-letter pair and blanked a cut line, and it picks per-cell drawing when a clip edge actually crosses the run (SPEC.md §6.1.2), so a caller may draw under an armed region without gating. Tracker's pattern columns were moved onto 8-pixel boundaries to earn it; **`WF_SNAP`/`OSAPI_WM_SNAP` (SPEC.md §11.94) is how an ordinary window earns it** — and it is **the DEFAULT now, with apps opting OUT** (§11.94.1): the stored bit is `WF_NOSNAP`, a clear bit means aligned, and `wm_create`'s `mov word [bx+W_FLAGS], 1` is the whole of how every window in the system gets it — no template word, nothing to remember at the twelve places a window is made. The published slot's contract is UNCHANGED (AL=1 still means "keep me aligned"), so no `.o88` was invalidated by the inversion and every historical AL=1 caller is now asking for something it already has. The price is 8px horizontal drag steps on every window that has not opted out.
**The other price was seven pixels at the left edge and it is REFUNDED
(SPEC.md §11.94/§11.95.2)**: the smallest x with `x+1` a multiple of 8 is 7, so
a snapped window could not sit flush against the screen, and §11.95's maximize
therefore left seven columns of desktop dither down the left of a window whose
right edge was flush. A snapped frame spanning the screen edge to edge from
x = 0 now draws **no left border** — a border separates a window from what is
beside it and at the screen edge there is nothing beside it — so its content
starts at 0, which is aligned, and the maximize is the whole screen. Derived
(`wm_flush`), never tracked, for §11.95's reason; the outline is three sides,
which is a drawing call *cheaper* than `gfx_frame`'s four; and the sharp edge
is `ui_grow`, which never re-snapped because **until this a width change could
not move a window's origin** — shrink a maximized window and column 0 stops
being an aligned origin, so it re-snaps and the damage union covers both rects
now. **The Disk window is the case that justifies it**: §11.94's "considered and left alone" was reasoning about `font_run` (which really would repaint each string's background over ground the fill already covered), and alignment is a different question — its file rows are at `fm_cx + 24`, so the ~40 `font_str`s that dominate `fm_repaint` all land aligned. Verified byte-identical to a full repaint after scrolling, on VGA and CGA. **§11.94.3 is the survey of which windows do not align their own TEXT** — the Disk window's own header (`+6`) and icon grid (`+78 × col`), Tracker's FT2 face, Tamegram's HUD, Fractal's status row, Paint's palette — each an offset to move rather than a rewrite, and `make SNAPAUDIT=1` is the instrument. **It applies on EVERY adapter, and it was mono-only for most of its life on a premise that measured false.** The premise was that what snapping buys is the single-store path above, which really is 1bpp-only — but **mode 12h is also eight pixels to a byte**, four planes of one bit, so an unaligned cell spills into a second framebuffer byte on VGA too and `font_char`'s planar path pays a second `GC8 Bit Mask` write and a second read-modify-write for it. Measured on a cycle-accurate 4.77MHz 8088, `tests/typebench`'s 40 keystrokes: alignment is worth **3.4% on Hercules and 9.4% on VGA** — *the adapter the gate excluded gains 2.8x the one it was written for* — and `gfxbench` prices the primitive on VGA at **12.5%** (`PAIR 10 aligned` 6984.93us against skewed 7855.51us). Note Pad on VGA was opening at content x = **61**, which is skew 5, the exact column typebench's skewed row measures. **The refutation had been sitting in §11.94 the whole time**: its `-icount` table already said 2.1% mono against 5.8% VGA, directly above the paragraph explaining the flag was a no-op on VGA, because the number was filed as a fact about `font_run` rather than about *alignment*. Removing the gate is a no-op on mono by construction and was checked that way — identical 7-step sessions against a reference kernel are **0 differing framebuffer bytes** on Hercules and CGA — and it made the kernel smaller (`.text` −18, `.cold` −4, no rung crossed). **`wm_snap` preserves FLAGS** — a package entry proc returns CF to the loader, and the `cmp` inside it left CF set, so asking to be snapped aborted the launch; that used to be visible on Hercules alone while loading fine on VGA, and with the gate gone every adapter reaches that `cmp`, so the `pushf`/`popf` is now the whole defence on all three.
...
**A FLOPPY skips its BPB re-read only when a CALLER says so (SPEC.md
§18.9.3).** §18.9.2 gave a fixed disk that for free, on a fact about the
hardware; a floppy can be swapped and no predicate the kernel can evaluate
answers it — §18.9.1's motor timeout is proof for one quiet switch and
nothing more, so a motor-gated batch revalidates on nearly every switch of an
A→B copy and buys almost nothing. The assertion is the caller's:
`OSAPI_BATCH_BEGIN` / `OSAPI_BATCH_END` (0x0380/0x0388), nesting. **And any
unlocking of the user interface ends it**, which is one store at the end of
`gfx_unlock` and is the whole safety argument: a bracket cannot be left open
across a moment when the user could reach the drive, because every way out of
a locked run — return, failure, a question, a swap prompt — comes through
there. So `dsk_batch_end` is optional and an unclosed batch is impossible
rather than merely rare. `dsk_bpbok` carries **2** for "banked inside a
batch" against 1 for "for ever", and the OUTERMOST begin clears every 2 — no
new array and no second opinion about which volumes are removable. Measured
on the hard-disk install: BPB **41 calls → 2**, the whole floppy side **356 →
163 (2.18x)**, ~92 s → ~50 s at PERFORMANCE.md's constants. The rule was
checked by reading `[dsk_batch]` out of the guest at the desktop, after a
driver load, after Mount and **at the swap prompt** — zero at every one.
...
`files_refresh` paints from the WINDOW's cache, `fdlg_home_go`
...
**A copy that will not fit is refused BEFORE it starts** (SPEC.md §22.5.2).
`fcp_xfer` streamed until the destination ran out and stopped, leaving a
**truncated file whose directory entry states the truncated length as though it
were the file's own** — measured, a 304,552-byte module onto a 264K volume lists
as `BANANA.MOD 260096`, and nothing on screen says so afterwards because the
verdict is a §59.5 toast that expires. `fcp_room` asks `dskw_dfree` (no disk
I/O, resident FAT snapshot) at the one moment the destination volume is already
current, so it costs no switch. Three deliberate properties: **a volume that
cannot answer is not refused** (a redirector with no `FSV_DFREE` behaves exactly
as before — a false refusal is worse than the bug), it compares **bytes not
clusters** so it is optimistic by one cluster's rounding on purpose, and a
**replace counts what it hands back**, banked at the `dskw_stat` the overwrite
prompt already does. `fcp_undo` is the net under it: from the moment the
destination is created, any failure deletes it, so **a failed copy leaves
nothing rather than something that looks like the file** — a failed replace
included, `dskw_write` having released the old chain by then. Not the RAM
disk's, found there only because a RAM disk is the easiest volume to fill.
...
- **The API slot numbers.** Everything above 0x01B0 moved **down 88 bytes**
  ...
  **Two cells have DIFFERENT numbers here and on `main`, and that is what a
  slot collision at this boundary looks like.** Both trees appended to the
  same tail at the same time: `main` put `OSAPI_GFX_BLIT1` at 0x03F0 and
  `OSAPI_VOL_SYS` at 0x03F8, which are `OSAPI_KEY_DOWN` and `OSAPI_FSX_SURF`
  here, so the incoming pair moved to **0x0418** and **0x0420** and the table
  is 131 slots. **The merge did not conflict on it** — the two sides added
  different NAMES, so `apps/os88api.inc` merged clean with two cells pointing
  at one another's addresses, and nothing would have said so until a package
  called one and got the other. That is docs/UPSTREAM.md's "assembles cleanly
  and every difference is silent" in its purest form, and the check is
  mechanical rather than a read: sort the `%define`s by address and look for a
  duplicate. Do it after every merge from `main`, not only when a conflict
  points at the file.
...
- **`cpudet.inc` + `xmem.inc` (§41).** CPU tiers, the A20 line and the store
  ...
  two maps above (SPEC.md §41.6). That line is on the **Memory** view, which
  is one click on the Task Manager's content away from the process page —
  worth knowing before reporting that there is no XMS bar.

  **`drv_tab` IS NOT THE WHOLE LIST OF LEGITIMATE DRIVER ROWS, and forgetting
  that killed the whole feature for two releases** (§41.12.5). `xm_row` is a
  `DRVR_`-shaped record deliberately outside the table — no Drivers tick, no
  publication slot, which is `drv_load_at`'s entire reason for existing — and
  `drv_call` grew a **range test on BX** against `drv_tab` after a field chase
  where it far-called into zeroed memory. The test asks *is BX inside
  `drv_tab`*, which is narrower than *is BX a driver row*, so the one call
  that reaches this row was refused before the driver was entered: `XMEM.DRV`
  was found, claimed, read and validated, then never called, and `xm_boot`
  freed it. On **every** machine that had a store. Three things hid it — the
  guard and the caller landed one PR apart (#80 and #81), §41.12.4 makes a
  refusal **silent by contract**, and the failure is pixel-identical to a
  machine that genuinely has no extended memory. The fix names the row rather
  than widening the range, because the guard's value is that it is exact. **The
  shape to recognise is a guard whose predicate is a PROXY for the invariant it
  means to enforce**, meeting the one case where the two differ — §18.7.3's
  `DV_CLASS` again.
...
full history. It stands at 112,128 today (`kern_small` 105,472), the fifth move having been *downward*
...
**An application's own state goes in `SYSTEM/APPDATA/`** (SPEC.md 19.9),
never beside the user's documents - a package's settings file in `DOCS`
is a file the user did not make and cannot explain. And the **core
packages ride the system disk too** (SPEC.md 24.3), a second COPY and
never a move: a single-floppy machine that has swapped to the apps disk
still has them, which is `ui_tm_open`'s reasoning (SPEC.md 28.3)
generalised.
...
because the dialog's default has to exist on whatever volume the user is on.
Its `MEDIA` held nothing for a long time (`os88disk.py --folder`, a folder
with no file naming it) on the reasoning that a boot floppy has no media —
and the cost of that was the **first `File ▸ Open` a new user ever ran
showing an empty list on a working machine**. It carries `OS8088.GIF` now
(SPEC.md §63): 466x110 of two-colour GIF, generated by `tools/os88logo.py`
at build time and never committed, drawn to the largest canvas the
*smallest* screen can show so that no adapter opens it cropped.
...
(SPEC.md §18.7.3/§62). `DV_PAD` became `DV_CLASS` — a spare byte in a 16-byte
...
(SPEC.md 72). `ETHER.DRV` publishes the whole socket surface as
`OSAPI_DRV_CALL` verbs, so the kernel learns nothing about TCP and a
machine with no card refuses at ONE fence rather than at every call site.
`apps/os88sock.inc` is that ABI written once, and `apps/os88line.inc` the
line discipline Telnet and the browser share - the `os88ui.inc` precedent
(SPEC.md 20.5.1), one source rather than two that can drift.
...
**A DRIVER'S SETTINGS BLOB BELONGS TO ITS `drv_tab` ROW, AND THERE ARE TWO**
(SPEC.md §51.9.1). `osapi_drv_cfg` carried one 34-byte blob for whoever asked,
which was the honest shape while exactly one driver had settings; the RAM disk
is the second, and a *shared* blob would have handed each of them the other's
bytes with **no diagnostic anywhere**, because the kernel does not read either.
It is keyed on the ROW and **not the class**, and that distinction is
load-bearing rather than pedantic: publication is per class and every other
fence in the driver layer keys on one, but `RAMDISK.DRV` and `NET.DRV` are
**both `DRVC_FILE`** — §18.7.3's `DV_CLASS` bug exactly, a structure that is
one-of-a-kind by accident meeting its second instance. Row 1 is the hard
disk's 34 bytes ('HD'), row 2 the RAM disk's 24 ('RD'); a published driver
with no row in the map is refused, which is what every driver but the hard
disk already got.

**A DRIVER'S PAGE CANNOT OPEN A FILE DIALOG, SO THE KERNEL OPENS IT FOR IT**
(SPEC.md §51.10, slot **0x0428**). `fdlg_open` fences on `inst_win_owner`: the
window it is handed must belong to a live INSTANCE, because the staleness
triple it banks is what stops a commit dispatching into a freed region — and a
driver has no instance and no such window, so every driver-side call was
refused and a page could ask for a name only by growing a typed name box.
`OSAPI_DRV_DLG` is that call with **the Control Panel's own window** supplied,
which is the honest answer rather than a way past the fence: the page IS drawn
in that window, the panel is a `KIND_CTRL` instance, and its folder memory
(§38.10) is the right folder for a page to open in. No line of `fdlg.inc`
changed. What the cell adds is the way back: the completion proc fdlg calls
must be the KERNEL's, because the panel's window has `W_SEG` 0 and
`wm_pkgcall` dispatches that as a **near call in KERNEL_SEG** — a driver's
offset there is a wild jump — so `drv_dlg_done` is resident `.text` and
far-calls the driver through `drv_call`. Three things hold it up: the row AND
the segment are both banked and compared, so a driver unticked while its own
dialog is up has the answer dropped; the request is armed last and disarmed
before the call, so a page may open the next dialog from inside this one's
completion; and an empty staged name is turned back into `SI = 0`, because
`fdlg_open` TESTS that pointer to choose between the caller's default and the
remembered one — `api_fdlg_open`'s own trap, one level up. **The caller must be
the RESIDENT driver**, the fence resolving a row from the calling segment, so a
page living in an overlay asks through its resident half and is called back
through it.
...
card — measured at **27 sectors**, ~6.4 s at the pre-fix 238 ms and ~0.65 s at
today's 24 ms per 512 bytes delivered (Set 24) — and then `drv_notice` opened the Control Panel on a failure nobody
...
### The RAM disk stopped being a harness (SPEC.md §62.9.9–§62.9.14)

`RAMDISK.DRV` was the file redirector's **test rig** — a volume with no
hardware, so every branch site the redirector added to the kernel could be run
on a cycle-accurate 8088 in a container — and it still is. What it also is now
is a RAM disk worth having: a size, a place, and a way for what is on it to
survive the power going off.

**It is TWO images, for §52.11's reasons.** `RAMPAGE.DRV` holds the Control
Panel page — layout, strings, paint, click, the dialog completion — and is
read off the system volume the first time somebody selects `Ram Disk` in the
panel, then freed at `DSV_CPCLOSE`. Three differences from `HDDTOOL.DRV` and
two of them are not obvious. **The trigger is a PAINT, not a click**, the whole
page body being over there; what stays resident is the NAME, which must
(`cp_list` draws it with `font_str` and the kernel stages it at attach), plus
page cells that are trampolines and draw a one-line apology rather than leaving
the pane blank when the disk is not in the drive. **The file dialog goes out
through the resident**, because `OSAPI_DRV_DLG` resolves a `drv_tab` row from
the calling segment and an overlay has none. And **teardown needs no shutdown
verb**: this page owns no window, so there is nothing of it on screen when the
panel has gone — the one thing that can outlive a close is a *dialog*, and that
is the resident's flag because the resident is who the kernel calls back.

**Ticking the row mounts nothing** (§62.9.13). Attach claims no memory and adds
no volume; Mount is the only thing that puts a drive on the desktop, and
`DRVV_READY` only restores a preserved image if the settings ask for one. That
is what makes the size and the location mean anything: both are questions that
can only be answered before the store is claimed.

**The store is chained extents through a table that is itself a heap claim**
(§62.9.10) — the harness's one-extent-per-file store had a hard 2KB ceiling,
which is right for a rig and wrong for a disk, and the byte-per-extent array
that first replaced it capped the whole store at 224KB. The chain is a **word**
per extent now and the **extent SIZE is derived from the store's**: the
smallest power of two between 1KB and 8KB that keeps the table inside
`RD_MAXEXT` (2,048) entries, which is §52.3's capacity table in miniature and
has §52.3's consequence — two stores of different sizes have different granules,
and an image records the one it was written with. `FSV_DFREE` is still a count
times a constant, which is the EXACT answer the original store was shaped
around and the thing not to give up. The table is **claimed first and freed
last**, ahead of the store it describes, because a store with no table cannot
be walked at all. A file caps at `RD_FILEMAX` (65,024) and that is arithmetic,
not policy: every offset inside a file here is a word — the *transfer*
arithmetic is 32-bit throughout, because the file API's counts are.
`rd_ext_seg` is the whole abstraction — *give me a segment through which extent
i can be read and written* — so the extended-memory path (a 4KB bounce and one
`OSAPI_XMEM_COPY` each way, always 1,024 aligned bytes) never spreads above
that one routine.

**THERE IS NO RAM DISK ON `kern_small`** (§62.9.15). A 128–256KB machine has
nothing to spare for a store made of the memory it is short of, so the
`drv_tab` row, the `'RD'` settings key and its 24-byte blob, `OSAPI_DRV_DLG`'s
body and — the one that is not obvious — **the Control Panel's entire
keyboard** (§31.9.2, whose only field is this driver's size) are all inside
`%ifdef KERN_BIG`, and both `.DRV` files leave the small disks. The **slot**
stays in both builds at the same offset and refuses, because one API table
serves one set of packages. The one divergence is `DRV_MAX`: the parallel link
takes `SYSTEM.CFG` bit 3 on big and bit 2 on small, so the driver bitmap does
not mean the same thing across the two — written down rather than worked
around, the two shipping separate disks. (Within ONE build the bitmap no
longer follows the table at all: `drv_cfgbit` gives each row a fixed bit, which
is what let SPEC.md §31.1 reorder the Drivers page to Sound / Hard Drive /
Ethernet / Ram Disk without re-ticking every settings floppy in existence.)

**And the CEILING is a fact about the machine, asked live** (§62.9.10.1).
`RD_MAXKB` is only the arithmetic limit (16MB) and is never what the page
offers: conventional is `OSAPI_MEM_AVAIL`'s largest free **run** less the
table, extended is what `OSAPI_XMEM_CAPS` will still hand out **and also**
capped by conventional room for the table and the bounce — the second cap being
the one that is easy to leave out and impossible to diagnose, since a store
above 1MB with no way down to it is not a store. It is asked live rather than
at attach because an application opening takes heap, and a size chosen against
a five-minute-old figure is a Mount that fails for reasons the user cannot see.
The rounding after the clamp goes **down** to a whole `RD_STEPKB`, or it could
put the size back past the ceiling the clamp had just applied.

**Three defects came out of the extended store together and all three are worth
recognising by shape. Only the first is proven to be a thing anybody saw.** The conventional figure is a **GATE, not a cap**
(§62.9.10.3) — conventional memory funds the chain table and the bounce, 12KB
between them, and the store is a megabyte up; subtracting it from the ceiling
made the extended store no bigger than the conventional one it exists to
escape, measured as **264K offered on a 640KB machine with megabytes free above
1MB**. §62.9.10.1's own sentence said "*and also* capped by conventional room
for the table and the bounce" — the intent was a gate and the code was a cap,
which is why the prose read as correct while the machine did not. **The fence
can refuse the driver's own block** (§62.9.10.4): `xm_chk` matches `XB_OWN`
against the caller, the block is claimed at Mount inside `drv_cp_call` (so it
is the kernel's, `0xFF`), and every later copy runs inside a file verb through
`drv_fs_call` — **which did not clear the dispatch stamp**, where `drv_call`
and `drv_cp_call` both did. **That one is LATENT and is not the reported
corruption**: a copy driven from a Disk window's own menu asks as `0xFF` too,
because Locator has no instance, so it matches by luck — measured, the same
116,085-byte round trip is byte-identical under `make FSNOSTAMP=1`, the build
with the defect put back. It bites the paths that DO carry a stamp: a package's
Save, a file-dialog commit inside an app, a worker. And **a refusal was
swallowed** (§62.9.10.2): `rd_stage_in`/`rd_stage_out` threw CF away, so a
refused stage left the bounce holding the previous extent and nothing reached
the store, with the directory perfectly correct about the name and size.
`[rd_xerr]` makes any such refusal `FERR_IO` instead of silence — a guard, not
a fix. **`drv_svc_call` and `drv_blk_call` still do not clear the stamp** and
that is recorded rather than fixed blind — nothing a sound or block service
does today asks the kernel for a stamped resource — which is §18.7.3's
`DV_CLASS` shape for the third time.

**A size step costs the size FIELD, and the gate is the interesting part**
(§62.9.11.1). Every other branch of the page's click ends in a whole-pane
repaint, which is the right default — one drawing path means a control's state
on screen cannot disagree with the resident's — but `-` and `+` are ~40
drawing calls at ~756us of arriving each to move one digit, and the pane
blanking and rebuilding twice a second is Part 1's double-draw flash with the
whole page as the flash. So a step is one opaque `OSAPI_FONT_RUN` over the
value box's own cells. **Four conditions gate it**: the size actually moved (a
clamped step draws NOTHING), the control is live, no verdict is on the glass,
and no new verdict arrived. The third is the one that is easy to miss and
impossible to see afterwards — `RSV_STATE` read-and-clears `RDS_ERR`, so an
error the last paint drew has *already* been consumed and a partial redraw
would strand it on screen — and what answers it is `[rp_eshown]`, **the record
of what was drawn**, written by the paint and by nothing else. That is §12.9's
`menu_bcell` discipline at the scale of one line. Measured: **14 frames /
229.7 ms with 13 flashing (worst 2,172 px) → 1 frame / 16.2 ms with 0**, and
`make RPSLOW=1` is the reference build the pane is compared against — **0
differing pixels of 13,320** across a `+`/`+`/`-` walk. Not one kernel routine
changed: `OSAPI_FONT_RUN` has been published since §6.1 and a page already
draws itself unclipped under the lock.

**…and the size can be TYPED, which is what finally gave the Control Panel a
keyboard** (§62.9.11.2, §31.9.2). Stepping is right for a nudge and wrong for a
decision: the ceiling is half a megabyte on a 640KB machine and `RD_STEPKB` is
8, so reaching a size the machine will fund is dozens of clicks. Click the box
and type. **A typed field and a reported one must not look alike, and here they
cannot** — the machine's answer is right-aligned, what is being typed is
left-aligned with a `_` after it, and that caret is the only thing on screen
that says which of the two you are looking at. **A commit goes through the same
`RSV_SIZE` the buttons use and then re-reads the state, so the field shows what
was TAKEN, never what was asked**: type 9999 on a 640KB machine and it settles
on 504. Esc and any other click cancel — `rp_click` ends the edit before it
looks at what was clicked — so a caret can never be left on a control that has
stopped existing.

**`cp_tpl`'s onkey word was 0 for the whole life of the panel**, so the entire
keystroke path had to be built for one box: a `W_ONKEY` on the panel's window
and a `DSV_CPKEY` cell a driver page may publish. **The key rides BX and not
AX**, which is the trap worth carrying away — `drv_cp_call` takes the CLASS in
AL (§31.9), the one dispatcher in the driver layer that can, so AX is exactly
the register a page argument may not use. In AX it assembles, boots, and hands
the page a class number as a keystroke: a field that takes the caret and then
ignores every digit typed into it, with no diagnostic anywhere. Cost: `.text`
+6, `.bss` +10, `.cold` +60 — **one cold rung crossed**, `KERN_BUDGET` spare
1,024 → 512.

**Preserve writes the volume as THREE blocks: the header and directory, the
chain table, then the arena verbatim** (§62.9.12). It was two, and the chain
table moved out of the first when it became a word per extent and stopped
fitting beside the directory — so a version 1 image is **refused** rather than
read, its byte chain decoding as extents that do not exist. `RDI_META` and
`RDI_CTAB` are 4,096 and the chunk 32,768 **because of `OSAPI_FILE_APPEND`**,
which refuses a file whose length is not a whole number of clusters — and the
store's own size is held to a multiple of `RD_STEPKB` (8KB) so that what is
LEFT at the last chunk is a multiple of 4,096 too. Lose that last one and the
final append of a 40KB store is fine while a 216KB one is `FERR_NAME`: a
size-dependent failure with nothing on screen to connect it to the size. **The
extent size is read and ADOPTED, not compared** — the geometry travels with the
picture, exactly as a FAT volume's cluster size does. **The row INDEX is in each
record**, which is what makes the format work at all — a handle here is `row
index + 1` and the parent column is a handle, so a restore that repacked rows
would renumber every parent link on the volume.

**§62.9.14 is what was actually driven and what was not.** The whole page, the
typed size, the round trip and the boot restore are proven on a cycle-accurate
5150 — a typed 48KB store, a real 17,982-byte package copied onto it, Preserve
As, Unmount, Load, and the file listed and readable again — and read back
**from the host by a parser written from the spec and sharing no code with the
writer**, which is what makes "18 extents for 18 needed, reassembling to a real
package header" a fact rather than the writer agreeing with itself. The
**extended-memory store is not exercised anywhere**, and that is this
container: `OSAPI_XMEM_CAPS` answers 0 on every machine here (MartyPC's are all
8088s; QEMU's 386 reports `xm_kb` 0 whatever `-m` says). The control greys
itself on any such machine and `rd_cfg_load` refuses a carried-in `Xms` — which
matters because the page greys BOTH radios there, so a stored `Xms` would
otherwise be a RAM disk that can never mount with the one control that could
undo it disabled.
...
### The toast is the menu bar's, not the window's (SPEC.md §59, `kernel/toast.inc`)

**One transient one-line message for the whole machine** — `Saved NOTES.TXT` —
at slot 0x0380, `ES:SI` + `CX` ticks, an empty string retires. Five apps had
re-rolled their own and the survey's first finding was that **only two of them
were toasts**: Tracker's and ModPlug's are *status lines* (they say what is
true now, are never *finished*, and so have no lifetime to share) and
...
on it *cannot* diverge between an incremental path and a full repaint. That
second one is why this was worth building: Note Pad's toast cleared
`[np_msg]` as it drew (or a drag re-showed `Loaded README.TXT`), so a full
repaint did not reproduce it — `pixcheck` measured **227 differing pixels** —
and the only thing reconciling the two paths was `np_sigsame` throwing the
fast path away and repainting the **whole content** on the next keystroke,
after every save and every load. **The erase was already written**: §12.9's
`menu_bcell` is both the composition buffer and the record of what is on the
glass, so composing *is* the diff and a toast expiring is *stop composing
it*. There is no fill in `toast.inc` and no `menu_inval`.

Four things are load-bearing. **`toast_show` stages and `toast_pass` (the
idle pass) draws**, which is what makes the context rule *any* — a callback
holds the lock, a worker holds nothing, `gfx_lock` is not reentrant — **and
`toast_now` is the exception that had to exist** (§59.4): a message put up
*before* a long operation (Paint's `Encoding...`, seconds of LZW inside a
...
and two were the wrong species: the `FERR_*` of the last file operation and
the `LD_*` of the last load (`Bad package`, `Protected`) — which §22.9 had
already worked out *"do not outlive their cause"*, building a
retire-on-next-interaction mechanism to make it true. That mechanism **is** a
toast, so the verdicts are toasts and the mechanism is gone: `FS_FERR` and
`FS_LDST`, their `[fm_lferr]`/`[fm_lldst]` mirrors and `fm_layout`'s copy,
`fm_stat_clear` and its "CF = 1 if something was actually cleared" contract
and its three call sites, four zeroing sites, and the ladder's shared table
walk — `.cold` −119. What is left is what a status line is *for*: a load
pending, the editor prompt, and the folder's size and free space. **And the
reason those bytes were per window dissolves rather than moves** — they were
per window because one global put `Bad package` on every open Disk window at
...
`[cp_dsave]` carried it to the Drivers page's caption, drawn *the next time
the panel was opened*, which is a verdict stored to be found later. It is said
at the moment it happens now, on a bar that is still there after the panel's
...
`wm_destroy`'s repaint cannot overdraw it. Deliberately NOT moved: the Sound
page's `cp_snd_err`, which sits next to the control it is about (§47 rule 6's
logic), the per-driver row states, which are status rather than verdicts, and
...
clamps every pen into it); reachable the moment a toast borrows cells, and
only for a message wide enough to reach back over a menu title's pen — about
40 characters, where everything the tree sends is half that. It holds the gfx
lock on the UI task while IRQ0 keeps running, so `[ticks]` advances and every
debug read answers: **the machine looks alive and never draws again.**
`menu_bpadc` clamps its own target now, at the routine rather than at its
callers. The round-trip test was the other half of the lesson: its message sat
at cells 33..49 while Locator's menus end near 26, so "0 differing bytes" was
a statement about blank space that read like one about the menus. It now
covers 11 cells of Locator's text and 21 of a Disk window's.

Cost, measured: `.text` +531, `.bss` +43, **one 512-byte image rung**
(`KERN_SIZE` 91,648 → 92,160, spare 2,560 → 2,048 — four steps, the
standard). Note Pad −239 bytes, Paint −138, which is heap and does not offset
...
### Closing asks first, and the alert is a PACKAGE's (SPEC.md §75)

**Every close in this system used to be final the instant the user asked for
it.** `app_close_win` is the one door — the close box, the app-name menu's
`Close`, the dock tile's context menu all arrive there — and it destroyed the
window, freed the region and released the record without the application
being told anything. So an application with unsaved work had no moment at
which it could say so. Three pieces, and **only the first two are kernel**:

- **`W_ONCLOSE`** (slot 0x0468, §75.1): a side table (`wm_oncl`, `wm_onwake`'s
  shape), asked by `wm_ask_close` from the TOP of `app_close_win`. **CF = 1
  means nothing at all happens** — not even the hide, because a window asking
  "save the changes?" has to stay on screen to be answered. It is the one side
  table `wm_init` zeroes, because it is read for EVERY window on its way out
  rather than only for one that installed something, so a stale word is a far
  call into a new tenant's image. `[wm_clask]` is a re-entry fence, saved and
  restored rather than set and cleared: a negotiator may raise a dialog, a
  dialog can reach a close path, and asking the same window again recurses on
  a 256-byte task stack. **It is asked ABOVE the ownership test**, so an
  UNOWNED window — the file dialog's species, and the alert below — can refuse
  its own hide, which is what makes the third piece possible.
- **`OSAPI_WM_CLOSE`** (0x0470, §75.2): how an app that refused finishes the
  job. **DEFERRED to the next UI pass, and that is the contract** — the caller
  is standing in the segment the close is about to free, so the far return
  would run its dispatcher out of memory the heap has handed back.
  `wm_close_pass` retires the negotiator before closing, so "no more
  questions" lives in one place rather than in every package.
- **`os88ui_ask`** (§75.3) — one line, up to three buttons (`OS88UI_ASAVE` =
  Save / Discard / Cancel), titled with the requester's own caption. **It is
  in `apps/os88ui.inc` behind `%define OS88UI_ALERT`, not in the kernel**, and
  the measurement is the argument: built as `kernel/ask.inc` first it cost
  **1,067 bytes**, and a slimmed version was *costed rather than guessed at* —
  fixed geometry (−153), one button set (−49), no title staging (−56), firing
  on the press (−80 and a §13.6 regression) — landing at a floor of ~800. A
  windowed dialog costs a window, a paint, a hit test, a key map and a
  completion dispatch, and none of those is the part you can take away.

**Out there it is SMALLER as well as free** (measured **607 bytes** of the
including package), and the three savings are all the segment boundary: nothing is
staged (the message and the title are near pointers in the caller's own
segment), there is no staleness triple (the alert *is* in the region that
would go stale), and the dismissal arrives through the alert's own
`W_ONCLOSE` rather than a collector in the UI pass. **`os88ui_arect` is a
FORMULA, not a layout** — equal-width buttons, which is why the set says
`Discard` and not `Don't Save`; measuring three labels was 153 bytes of the
kernel version. **`os88ui_aclose` answers CF = 1** although its job is to let
the close happen: by then it has already destroyed the window and dispatched
the cancel, and letting `app_close_win` hide a freed record is the one thing
that would not survive. And **refuse-and-raise covers minimizing**
(`OSAPI_WM_SHOW` is un-hide and raise in one), which is what this has instead
of modality (§75.3.1).

**The kernel pays 222 bytes and BOTH BUILDS have it** — `.text` +194, `.bss`
+28, `.cold` +0, no rung crossed on either guard, nothing inside an
`%ifdef KERN_BIG`. The
first draft had `ask.inc` as `kern_big`'s alone and a save-and-close fallback
on `kern_small`; that distinction is gone with the code that needed it.

**Note Pad is the first consumer and its dirty test is a CHECKSUM, not a flag**
(§27.15). A flag has to be set by nine mutation sites and answers DIRTY for a
character typed and taken straight back out; a shadow copy costs 16KB of the
one claim the app cannot do without. Fletcher's two sums plus the length are
three words of bss and one walk at the moment of closing. `[np_named]` is the
second question and is **not** "has a name" — `np_defname` seeds `NOTES.TXT`
so Ctrl-S always has somewhere to go, so an unnamed note gets the Save As
dialog rather than that file being written over. `tests/dispclose.py` is the
gate, on all three adapters and on `kern_small`.
...
**`make zcheck` is the gate a Frotz change is actually checked with**, and it
had been left out of this file entirely: it plays every story to a script and
diffs the transcript against `dfrotz`, which is the only thing that exercises
an interpreter — a story is minutes of typing by hand, and
`tools/zharness.py <story> --repl` is that hand route when you want it.
`zcheck` **cannot see a graphics defect**, which is why `zgfx` exists beside
it rather than under it: a story that draws a quote box and loses it prints
exactly the same characters as one that keeps it. `zgfx` checks every row the
interpreter claims against the pixels under it, the same across a repaint, and
each story's opening screen against the real Frotz's; **`make zpic`** builds
the v6 picture fixture it ends with, and **`make zscreens`** re-takes the
golden screens — the only part of any of this that needs `frotz` and `pyte`
installed on the host.

### Microsoft Word — a real Opus, and it rides no shipped floppy either (SPEC.md §68, `apps/word/`)

**It is not a port**, the same way Frotz is not a port of Frotz: this is an
independent 8086 reimplementation of Microsoft Word for Windows 1.1a
("Opus"), with the menu bar, ribbon, ruler, status line, dialogs and key map
taken verbatim from the Computer History Museum's Opus source release, and
the document engine is Note Pad's (§27) transplanted under `wd_`. Draft view
is the whole of the renderer's ambition — bold is a double-strike, italic a
pre-rendered sheared glyph table blitted per run, everything else a drawn
rule — because os8088 has one 8×8 fixed font and no styles in it, and Opus
itself shipped View > Draft for exactly this machine.

**The file it writes is a real `.DOC`, not a private format wearing the
name** (§68.4): a 384-byte FIB header, text, CHPX/PAPX FKP pages with bin
tables, a one-style STSH, a plcfsed and a DOP, laid out from the Opus
headers' own structures — so a file this package writes is one Word for
Windows 1.x/2.x could open, and one it opens can have come from there. There
is no Word here to check that against, so the format has a **second,
independent reader** (`tools/wordfmt.py`, host-side Python) that parses what
`tools/os88doc.py` writes and `wd_docsave` writes, from the same spec and
sharing no code — `make wordcheck` is the round-trip gate.

**It follows Frotz's precedent exactly** (§68.5): `WORD.O88` rides its own
dedicated floppy and never the shipped apps disks, with an empty `DOCS\`
folder for the user's documents and `WELCOME.DOC` at the root — a real,
deterministically generated `.DOC` (`tools/os88doc.py` from
`apps/word/welcome.wtx`) that exercises the formatting the same engine
renders. `make worddisk` builds `build/word.img` / `word720.img` /
`word360.img`; `all` builds none of it, same as Frotz's story disk. `make
xt-word` and `make 386-word` are the two 86Box machines (period XT at 640KB,
comfortable 386DX), both with the Word disk in B: instead of the apps disk.

**One mechanism in it is unfinished, and it is written down rather than
hidden** (§68.10): `WORD.OVL` is a §2.8-style on-demand module — code that
ships as a file beside `WORD.O88`, loaded into a heap claim on first need,
addressing the package's own data through `DS` exactly like resident code —
which is how a package whose image+bss cannot cross 64KB (§33) grows a
feature without spending the segment it has none of left. The load path is
proven: found, read, far-called, dispatched, returned. **The shim that calls
back into a UI-facing resident routine was the open question, and it no
longer reproduces.** Routing `wd_saymsg` through one used to freeze the app
with every static explanation checked and cleared and the cause unknown;
re-run against the tree at §5.4.2.1 — three variants, seven round trips —
all three work, and the likeliest explanation is that the defect was
`gfx_blit1` restoring `DS` after its nest count rather than before. That fix
is `c257e9f`, which is this branch's own merge base, so it has been in this
tree the whole time the note said otherwise. §68.10 is the record; a feature
may move into the overlay now. **§73.14 is the same design for a COMPILED
module**, proven the other way by `tests/covl`, and it names the one thing
that does not carry into C unchanged: a `call`/`retf` shim puts a compiled
routine's arguments two bytes further from its frame than every reference to
them says.
