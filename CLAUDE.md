...
| **[docs/KERN-SMALL-CUT-PLAN.md](docs/KERN-SMALL-CUT-PLAN.md)** | cutting `kern_small` down for the 128KB machine, or answering "what does a 128KB system not need". **Design not started**; it is a research document, and its finding is that the requester's own list is the wrong 20% of the kernel. A 128KB machine's free heap is **32.5 KB** today and the ask is 70 - `KERN_SIZE` 96,256 -> 57,856, a **39.9% cut** - while sound, the hard disk, Ethernet, XMS, the PS/2 mouse and the VGA are worth **~4,700 bytes together**, most of it `driver.inc`'s loadable-driver machinery rather than any device: every card is already a `.DRV` costing zero resident bytes, and the VGA, the planar decoder and XMS were gated out some time ago. **The file system and the window system are 69% of the code**, so a 40% cut cannot come out of the hardware column. There is no fat symbol either - the largest in the kernel is `osapi_table` at 1,256 bytes and the second is `kmain` at 217 - so the cut is whole FEATURES, and §2-§5 prices every one of them off a symbol map that reconciles exactly to the section lengths. **The best lever keeps the features rather than deleting them**: `.cold` is 34,531 bytes, `mod.inc` already has 10,536 of it out of RAM as on-demand modules, and the file dialog and Cut/Copy/Paste are **4,649 more** by the same route - a figure this file's §6 first put at 12,997 and docs/KERN-SMALL-MODULE-SPLIT.md corrects. §6.1's product decision still stands. **§7 is the floor nobody had to notice**: `.ovlw` is loaded onto the FAT window and spills through the mount buffers, so those may shed **2,816 bytes between them and not one more**, which caps three of the most attractive data cuts at half their row - and it relaxes once the features in the overlay are gated, so the order is features first and buffers second. **The whole list, taken, reaches ~65 KB and not 70**, having given up file writing; keeping the file dialog and Cut/Copy/Paste as modules instead costs 8.4 KB of heap, which is §8.1's trade to put to the owner. Two refusals: the damage-rect layer is the redraw architecture and not a nicety, and `SHEET.O88` - 48,352 bytes, the largest package in the tree, unloadable on a 128KB machine today - runs at 65 KB with 13 to spare, so the last 4 KB may buy no program at all |
| **[docs/KERN-SMALL-MODULE-SPLIT.md](docs/KERN-SMALL-MODULE-SPLIT.md)** | moving more of `.cold` behind `mod.inc`'s on-demand mechanism, **on `kern_small` alone** - `kern_big` keeps every byte resident and its cylinder-run boot read unchanged, which is free, because a module is CUT OUT of `kernel.bin` and `MODC_START` is exactly where the image ends. **Design not started.** Its finding is that **two of the four candidates are refused by the mechanism itself**, so the net is **4,649 bytes and not the 12,997** docs/KERN-SMALL-CUT-PLAN.md §6 claimed off a sum of their `.cold`. **`mod_need`'s own transitive cone is 155 symbols in 7 files - 16,880 bytes, 48.9% of `.cold`** - and `assoc.inc` is INSIDE it: `mod_need -> drv_mounted -> dsk_chdir_q_x -> dsk_chdir_x -> disk_mount_x -> asc_lookup_x`, so loading any module can mount, and a mount calls associations. **`diskw.inc` is not the write path, it is the by-name file I/O layer** - `mod.inc` calls `dskw_read_x` TO LOAD A MODULE, `driver.inc` and `loader.inc` call it to load a driver and a package, CTRL.DRV and CLONE.DRV far-call `dwf_dskw_*` from inside their own images, and it has 33 entry points against `MOD_NENT`'s 8. What is left is `filecp.inc` (5 entries, clean) and `fdlg.inc` (9 bodies from 11 names), the second needing `MOD_NENT` raised to 16 and `apps/os88ui.inc` LIFTED OUT of its `.cold` extent first - **that include is ~1,140 of fdlg's 4,292 and five other files need it, so it stays whether fdlg is moved OR deleted**, which is a correction to the deletion row too. §1 is the cost model off the FORMAT.DRV precedent (a module is a SECTION, not a file - `diskw.inc` already contributes to both `.cold` and `.modf`), §4 is the work file by file including the three hardcoded lists that must move together, and §8 records the measurement error worth not repeating: the first call-graph pass **could not see `call far COLD_SEG:label`** and read fdlg at 2 entries instead of 11. **§9 IS THE DECISIONS AND THE WAVE ORDER**, and it is not the order that was proposed: `assoc.inc` is GATED rather than moved, and it goes FIRST, because gating it needs no new mechanism at all and is worth **5.5 KB** where either module is under 3 - `asc_use_x` claims `ASC_KB` = 3,072 bytes under a kernel tag rather than a purgeable class, at the BOOT mount, and nothing frees it. That also means **today's 32.5 KB is optimistic and a bare desktop really has ~29.5**, and that nobody has ever pointed `tests/kernresident.py`'s `mem_tab` walk at `kern_small` - the cheapest unexamined lever in either document. Three waves reach **7,680 bytes of footprint and ~13.5 KB of usable heap**, 29.5 -> 43.0 |
...
| **[docs/BROWSER-PLAN.md](docs/BROWSER-PLAN.md)** | the browser (§71) — the renderer's steps, and `tools/htmsim.py` is its reference implementation. **§14 is the field's three (issue #137) and its finding is about that model**: htmsim renders the reporting page CORRECTLY, so "the model and the 8086 must agree on the text" passed all three — the doubled last line and the dropped table anchor live BELOW the text, in which document offsets a line covers and in what a click resolves to, and the `..` was in `br_resolve`, which the model does not have because it never follows a link. A reference implementation is a check on the parts it implements and silent on the rest. The doubling was one `</p>` away from the suite for the whole of step 1: `br_layout` tested `D_END` one instruction after the emit that `.fin` repeats, and every fixture ended `</p></body></html>` so the guard suppressed the second copy |
...
`xt-cga`, `xt-hercules`, `xt-ega`, `xt-multimon`, `xt-sound`, `xt-sound-1.44`,
`286`, `286-525`, `286-sound`,
