...
| **[docs/FIELD-NOTES.md](docs/FIELD-NOTES.md)** | bugs reported off real hardware, one numbered entry each — eight still open (3, 10, 14, 19, 24.2, 28, 32, 40) with what has been ruled out and what to try next. **40 is live and this branch owns part of it**: Cyclone overflows its task stack, the mechanism is the BIOS beep nesting `int 09h` that `cy_kbdrain` was written against — and GFX-EMBEDDABLE-PLAN's wave 5 took `cy_worker` **66 → 86 bytes**, which `tests/unit/t_stkclass.py` reads as **1.48x → 1.28x, the thinnest margin in the tree**; the closed ones are a paragraph naming the fix and its §, kept because code and SPEC.md cite the numbers |
