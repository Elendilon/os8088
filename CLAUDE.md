...
| **[docs/MONO-RECLAIM-PLAN.md](docs/MONO-RECLAIM-PLAN.md)** | asking what a machine with NO VGA could do with the VGA code it carries and cannot enter. **Design not started**; it is a handoff, and it is measured rather than argued: **1,771 bytes of VGA-only `.text` on kern_big and ~1,207 on kern_small**, of which `vga_solid_rect` is held aside because `gfx_fill_raw` FALLS THROUGH into it and grouping it puts a `jmp` in the hottest fill path in the system. **The bytes cannot go back to the heap** - SPEC.md 39.22's ladder settles that, and moving hot drawing code far has already been tried and abandoned - so the space is reusable IN PLACE and the question is what a mono machine is refused today for want of exactly that many bytes. Four candidates, and the best two are code that already exists behind an `%ifdef`: SPEC.md 5.6.4.4's `gfx_line_fast` (**686 bytes, 4.9x, and mono-only code by construction**) and 5.9.6's `gfx_blit1` - kern_small's ~1,048 movable bytes buy ONE of the two. The gate is `[vid_avail] & VID_A_VGA` and never `[vid_mono]`, because a VGA machine can be SWITCHED to mono and may never give its bytes back - which is why only one side of the kernel is ever reclaimable. Read its 4.4 before quoting PERFORMANCE.md Set 64 at unaligned text: **Set 64 predates SPEC.md 6.1.11** and Set 100 is what is left |
