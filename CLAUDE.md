...
| **[docs/plans/GFX-EMBEDDABLE-PLAN.md](docs/plans/GFX-EMBEDDABLE-PLAN.md)** | moving kernel drawing code into the APPS that use it. **RESEARCH, nothing built** - `gfx_embeddable` is an app-side library in `os88ui.inc`'s shape, opted into capability by capability, with LINE as its first customer and not meant to be its only one. Its premise is the owner's: *duplicate code only used by apps that monopolise the machine anyway, instead of permanently spending kernel RAM on them* - and two measured facts turn that from a size trade into a straight win. **An app-side rasteriser is FASTER** (24.6 us a pixel against `gfx_line`'s 31.6 with the arrival removed), and **`kern_small` has never had `gfx_line_fast`** - SPEC.md 5.6.4.1's 4.9x interval walk is `%ifdef KERN_BIG` because it is 647 bytes, so in a library the floor machine's Paint can BUY a capability the kernel never gave it. The prize is **1,377 `.text` + 69 `.bss` on kern_small, 2,520 + 80 on kern_big**, and the owner's prediction about where the difference sits is confirmed to the byte: **1,143 bytes of kern_big is speed optimisation**, 647 of it `gfx_line_fast`. Its 1.2 is the finding nobody was looking for - **the kernel carries TWO 1bpp walkers**, `gfx_line_mono` 286 and `gfx_lstep_mono` 212, one capability written twice because the line and the walk were built at different times, which is exactly what a lattice where WALK is built ON LINE deletes. What it was blocked on is gone: a windowed package has no framebuffer and never will (`FSI_SEG` is fsx-only and 53.7 makes the two exclusive), `gfx_spans` is the wrong slot (no body on small, refuses under an armed clip on big - **stop re-deriving this**), and **`gfx_blit1` is the right one** - it takes `cur_unlazy`, the display AND `wm_clip_rows`, so it is legal from a window, and nine shipped small-disk packages already call it, so its kern_small body is not this plan's cost to carry. **Its 3 is the section to read** and its first draft was wrong twice: the commit is a **FOUR-WAY** choice, all published and all measured, and the two units are the point - a BAND is priced by its AREA (`GFX_BLIT1` 0.40-0.77 us/px opaque, `ICON_DRAW` ~39-47 transparent) and a LINE or PIXEL by the INK it lays down (`GFX_LINE` 37.1, `GFX_PIXEL` 640.87). So on the 127x32 line the tree already benches, the opaque band is 3.1 ms, direct `GFX_LINE` 4.7, and **the transparent band 158** - for a SPARSE figure the masked commit is 34x worse than the thing it was meant to rescue, and the published one is capped at 16x16 by a **66-byte staging buffer** rather than by the renderer (`icon_draw` itself has no cap; `icon_draw_x` refuses). **3.2 is the fourth option and it is now MEASURED - PERFORMANCE.md Set 132**: 5.6.7's argument for a resumable walk is about the KERNEL holding the state, so an app that holds its own calls `OSAPI_GFX_LINE(p_prev, p_now)` a frame and the erase replays it exactly (5.6.2 makes the pixel set a pure function of the endpoint PAIR) - no band, no shadow. On a Hercules 5150 the walk is LINEAR and agrees with 5.6.8 to 3% (4,013 us for eight blocks + 1,230 a pixel across them) while `gfx_line` is FLAT (986 us a call at two pixels, 1,147 at eleven), so the crossover is **4.2 and not the 2 predicted** - the error being a short line's fixed part, ~1,150 us where the plan borrowed 714 from a 127-pixel row's intercept. And the same run's **`GFX_PIXEL` at 539.52 us is a THIRD route that wins below ~1.3**, which is the finding that matters: **`gfx_lstep` is the best of the three only in a WINDOW of 1.3 to 4.2 pixels a block a frame**, so its 537/641 bytes buy a window rather than a category, and what Cyclone and Missile actually step is one counter and one rebuild away from deciding wave 4. **No program in the tree needs a full-window shadow.** Its 6 is `OSAPI_GFX_PIXEL`, DEFERRED to after the line waves by decision and priced there so the deferral is informed - **12 bytes** (`a pixel is a 1x1 solid rect`), shipping callers including `os88_gfx_pixel()` in the C header, and a confusion that is about its **cost** rather than its existence. **Waves 0-3 take nothing out of either kernel** |
...
- **Text is `font_run`, not `font_str` (§6.1, §6.6).** `font_char`/`font_str`
  ...
  *visible* on the target machine. `font_run` draws the ground and the glyph in
  ...
  finished** (§6.6.5): the registry stands at 60 sites in 20 files, every one
