...
- **Memory budget — read `docs/KERNEL-MEMORY.md` before spending any.** Two guards bind the kernel, and they bind *different* things. **Guard 1**: the whole kernel — image, `.bss`, the cold segment, the FAT window, the disk buffers and every task stack — is one contiguous span from `KERNEL_SEG`, measured against `KERN_BUDGET` (79KB; it has moved four times, each asked for and granted — the constant's own comment in `kernel.asm` is the history). **Guard 2**: `.text` + `.bss` must fit the kernel's own 64KB *segment*, which no budget can raise — 16-bit offsets. Two mechanisms relieve guard 2 and neither relieves guard 1: the **boot overlay** (`.ovl`, SPEC.md §2.5 — run-once init code landed in the FAT window and overwritten by the first mount, costing nothing at all) and the **cold segment** (`.cold`, SPEC.md §2.6 — resident code with a CS of its own, DS still `KERNEL_SEG`). A near call or branch crossing either boundary is a bug NASM cannot see; `tools/os88ovlchk.py`, run by `make`, refuses it. The menu save-under is a heap claim rather than a reservation. **Growing past the budget is a decision to take with whoever asked for the feature, not a build fix.** The heap starts where *this build's* kernel ends, so it moves whenever the kernel does. Task stacks are **256 bytes** with a `SCH_MAGIC` canary at the bottom of every slice, checked at each switch away — an overrun halts in `sch_stkdie` instead of corrupting the next task's stack. Re-run the fill probe (KERNEL-MEMORY) before trusting a smaller number, and remember the probe under QEMU understates a real BIOS's interrupt stack use — SeaBIOS services its interrupts on an internal stack; an IBM BIOS lands them on whichever task stack is current.
...
full history, and it stands at 80,896 (79KB) today. The 64KB *segment* limit
(guard 2) is untouched and unraisable: 16-bit offsets. `BOOT_RELOC` moves
