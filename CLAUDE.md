...
| `MOUPRIV=1` | run the WHOLE mouse ISR on a **private stack**, so what it costs the task it interrupted is the six bytes the CPU pushed and nothing else (docs/STACK-SLOTS-PLAN.md §4.2). It covers BOTH mouse ISRs - the serial one and `mou_p2_isr`, the PS/2 aux port's (§9.9), which the first version MISSED and a Packard Bell 286 caught on the glass: "mouse ISR, own stack" read 0 there while the mouse phase still added 16 bytes to a slice. Measured with `STKDIAG=1` beside it: the ISR is **~54 bytes on VGA, 30 on Hercules, 23 on CGA** - it is the ADAPTER that sets the depth and not the mouse, the PS/2 ISR reading 52 on a 286's VGA against the serial one's 54, because `cur_move`'s 1bpp path is the shallow part - so a class scheme is cut from the VGA figure - and they come off every task slice for **21 bytes of `.text`** — an 8-byte swap at BOTH vectors, which is the duplication that buys it, plus a 5-byte restore at `mou_eoi`'s single `iret`. `mov [cs:x], sp` and `mov sp, imm16` need no register, which is what makes a swap possible at a gate where every register is the interrupted task's. **It needs no re-entrancy guard** and that is a property rather than a hope: `mou_isr` runs IF=0 from the gate to the `iret` and never `sti`s, so it cannot interrupt itself and IRQ3/IRQ4 cannot interrupt each other — where the tick's chain needs a busy flag precisely because a real BIOS `sti`s inside it. The two moves **compose non-additively**: IRQ4 can land inside the ROM's chain, so one slice can carry `sch_isr`'s frame AND the ROM AND the mouse at once, which is the 130-byte excursion STACK-SLOTS-PLAN §10.3 caught — either move removes that product case |
