...
| **[docs/plans/completed/TEXT-PLAN.md](docs/plans/completed/TEXT-PLAN.md)** | touching how TEXT reaches the screen. **Reach for `font_run`, not `font_str`** — the transparent pair is the double-draw flash. **STAGE 4 IS DONE**: the registry is 189 → 37 sites and every line left carries a REASON rather than a queue position — four of the seventeen still spell it `backlog:`, and docs/plans/completed/TEXT-PLAN.md §4.4 says why none of those four is a conversion — so a NEW transparent call site is now a claim to argue rather than a queue to join |
| **[docs/plans/GFX-REWORK-PLAN.md](docs/plans/GFX-REWORK-PLAN.md)** | touching the graphics layer's SHAPE rather than one routine — the per-call cost, the segment layout, and what the composer that started it is now waiting on. **Design not started**; it is a handoff, and PERFORMANCE.md Sets 88/89 are the two traps it exists to prevent: a design that removes calls without beating them, and a **ratio of totals passed off as a measurement of a design**. `gfx_hline` spends 3,235 cycles on six pixels and 4,112 on 312 — the width barely matters, and that is what the rework is for |
...
python3 tools/os88bisect.py classify <row>   # WHY A ROW FAILS, without
              ...
              #   (docs/plans/completed/SOAK-PARALLEL.md 10). It is the classification
...
python3 tools/os88soak.py start    #   detached (docs/plans/completed/SOAK-PARALLEL.md).
python3 tools/os88soak.py status   #   `make test-soak` runs the same rows
              ...
              #   (docs/plans/completed/SOAK-PARALLEL.md 14.3), so a restart from zero
