...
currently runs 0x00600 through 0x14DFF, and the budget's ceiling is 0x15600.
...
left **4,608 bytes spare — nine 512-byte steps** (four now: SPEC.md §14.1's
Timer redraw was the first of the QOL work to land, and move 11 handed a
...
| 11 | 86,528 → **86,016** | *nothing — this one gives back too.* SPEC.md §53.6.1's XMS desktop stash was removed (a snapshot cannot restore a desktop that moved while the bracket ran), and the 512-byte step it cost goes back with it rather than becoming slack |
...
moved costs a whole step. Even with four steps left under the budget, **moving data
...
|---|---:|---|
| image (`.text` 45,899 + `.bss` 3,766) | 50,176 B | all resident kernel code in the kernel's own segment, its read-only data, and its scratch |
| cold code | 19,968 B | 19,911 bytes with a CS of their own: the five file modules and the Control Panel, and since SPEC.md §53.6.1's removal nothing else at all |
...
| **total** | **83,968 B** | of an 86,016-byte budget — **2,048 B spare**, the fifth move's standard |

Each rung is its contents rounded up to a whole 512 bytes, and the remainders
are the only slack anywhere in the ladder: 511 bytes on the image, 57 on the
...
`FAT_SEG` 0x1180, `LOW_SEG` 0x12A0, `HEAP_SEG` 0x14E0.
...
### The image — `.text` 45,899 B + `.bss` 3,766 B
...
— 50,176 B, so 511 bytes of the rung are rounding remainder.
...
stopped being true with SPEC.md §2.6: `.cold` holds 19,911 bytes today, and
...
65,810 bytes of kernel code — `.text` 45,899 plus `.cold` 19,911 — module by
...
- **The file system is 37.0% of the kernel.** `disk` + `diskw` + `files` +
  `filecp` + `fdlg` + `loader` + `assoc` come to 24,361 bytes, nearly twice
...
- **30% of the kernel's code is not in the kernel's segment, and it is now
  exactly the file modules and the Control Panel.** Those six are 19,911 of
  19,911 cold bytes — `fsx`'s XMS desktop stash used to be a seventh at 190,
...
| theme | bytes | share |
|---|---:|---:|
| the file system, end to end | 24,361 | 37.0% |
...
|---|---:|---:|---:|---:|---:|
| `files.inc` — the Disk window (§22) | 821 | 6,111 | **6,932** | 316 | — |
...
| `assoc.inc` — file type associations (§54) | 2,608 | — | **2,608** | 43 | — |
...
| `loader.inc` — the package loader (§21) | — | 735 | **735** | 58 | — |
...
| **total** | **45,899** | **19,911** | **65,810** | **3,766** | **7,748** |
<!-- END generated table -->
...
- **`files.inc` has 821 bytes left in `.text` and 6,111 cold.** What stayed
...
| tenant | bytes | |
|---|---:|---|
| `files.inc` | 6,111 | the Disk window (§22) |
...
| **total** | **19,911** | of a 19,968-byte rung |
