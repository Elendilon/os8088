...
|---|---:|
| guard 2, the segment | **10,920 B** |
| guard 1, the budget | 9,728 B |

At 54,616 bytes of image **guard 1 is the tighter of the two**, and by a
...
|---|---:|---|
| image (`.text` + `.bss`) | 54,784 B | all kernel code, its read-only data, and its scratch |
...
| **total** | **71,168 B** | of an 80,896-byte budget — 9,728 B spare |

The image rung is `.text` (51,147) + `.bss` (3,469) = 54,616, rounded up to a
whole 512 bytes; the 168-byte remainder is the only slack anywhere in the
...
The ladder lands on these segments: `KERNEL_SEG` 0x0060, `COLD_SEG` 0x0DC0,
`FAT_SEG` 0x0E80, `LOW_SEG` 0x0FA0, `HEAP_SEG` 0x11C0.
...
> **heap KB = what int 12h reports − 71**

`KERN_END` is 4,544 paragraphs = 72,704 bytes = 71KB, and the heap starts
there. Checked against a live machine: QEMU with `-m 1M` reports **639KB**
and the Task Manager shows **568KB** of heap. Re-derive this after
...
alignment invariant below) — 54,784 B, so 168 bytes of the rung are rounding
remainder.
...
| ...and where it stands now | 79 KB | **69.5 KB** (71,168 B) |
...
The 54,616 bytes of image, module by module, and one level down inside each.
...
- **The file system is 37.5% of the kernel** — `disk` + `diskw` + `files` +
  `filecp` + `fdlg` + `loader` come to 20,468 bytes, half again as much as the
...
| theme | bytes | share |
|---|---:|---:|
| the file system, end to end | 20,468 | 37.5% |
| the window system and its furniture | 12,484 | 22.9% |
| hardware: clock, mouse, sound, CPU, XMS, drivers | 8,272 | 15.1% |
| drawing: adapters, primitives, glyphs, icons | 7,649 | 14.0% |
| the kernel proper: scheduler, heap, API table | 4,130 | 7.6% |
...
| | bytes | of image |
|---|---:|---:|
| **`files.inc`** — the Disk window (SPEC.md §22) | **6,489** | **11.9%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *309* | |
| **`wm.inc`** — the window manager (§11) | **4,675** | **8.6%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *523* | |
| **`diskw.inc`** — the FAT write path (§18.4-18.6) | **4,238** | **7.8%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *221* | |
| **`fdlg.inc`** — the Standard File dialog (§38) | **3,263** | **6.0%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *135* | |
| **`ui.inc`** — the UI task and the event ladder (§13) | **2,133** | **3.9%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *53* | |
| **`menu.inc`** — the menu bar and pull-downs (§12) | **2,012** | **3.7%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *10* | |
| **`vgabb.inc`** — the software renderer / back buffer (§32, §39.5) | **1,563** | **2.9%** |
...
| &nbsp;&nbsp;&nbsp;&nbsp;the dirty rect | 89 | |
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *27* | |
| **`kernel.asm`** — the API table, entry points, `kmain` and the segment shims | **1,512** | **2.8%** |
| &nbsp;&nbsp;&nbsp;&nbsp;the API jump table and its X/N stubs | 1,096 | |
...
| &nbsp;&nbsp;&nbsp;&nbsp;init and unhook | 23 | |
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *299* | |
| **`icons.inc`** — the icon renderer (§10) | **1,343** | **2.5%** |
...
| **total** | **54,616** | |<!-- END generated table -->
...
```
guard 2  .text + .bss   54,616 / 65,536  10,920 bytes
guard 1  KERN_SIZE      71,168 / 80,896   9,728 bytes
```
