...
|---|---:|
| guard 2, the segment | **6,179 B** |
| guard 1, the budget | 5,120 B |

At 59,357 bytes of image **guard 1 is the tighter of the two again** — the first time since hard-disk support. The segment used to run out first, and hard-disk
...
The image rung is `.text` (54,724) + `.bss` (4,633) = 59,357, rounded up to a
whole 512 bytes; the 6,179-byte remainder is the only slack anywhere in the
...
### The image — `.text` 54,724 B + `.bss` 4,633 B
...
alignment invariant below) — 65,536 B, so 6,179 bytes of the rung are rounding
remainder.
...
The 59,357 bytes of image, module by module, and one level down inside each.
...
| the window system and its furniture | 11,779 | 19.8% |
...
| &nbsp;&nbsp;&nbsp;&nbsp;*.bss scratch* | *1,204* | |
| **`wm.inc`** — the window manager (§11) | **4,675** | **7.9%** |
...
| **`desk.inc`** — the desktop and volume zones (§14/§26.1) | **759** | **1.3%** |
...
```
guard 2  .text + .bss   59,357 / 65,536   6,179 bytes
