...
| **Tank Attack** | the CGA/Hercules shadow and its HUD template, as ONE claim off a ladder — `TK_SHKB` 18KB, else 17, else 16 (SPEC.md 85.3.5.1). It was a flat 32KB until the template stopped being a second 16,000-byte frame buffer and became a span store, which is what put this package on the 128KB machine's disk |
