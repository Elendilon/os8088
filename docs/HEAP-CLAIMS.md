...
| `MEM_K_DRV` driver image | per loaded driver | **PINNED (forever)** | its base is the driver's CS. A package REGION is a CS too and is no longer forever (§66.6.1); a driver image is docs/plans/HEAP-UNPIN-PLAN.md's piece D and is not built |
