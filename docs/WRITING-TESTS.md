...
(docs/SOAK-PARALLEL.md). This file is how to add a row to it without
reinventing the six things that already exist, and without producing one of the
four failures that keep recurring.

**It is a rules document, and every rule in it is a bug somebody shipped.** The
named incidents are here on purpose: a rule with a corpse attached gets
followed, and one without gets argued past. docs/TESTING.md is the other half —
what each emulator *can* show — and docs/MARTYPC-DEBUG.md is the instrument's
manual.
...
Write the failing case first. Not as a formality — as the only question that
decides whether the row is worth having.

A row that cannot fail is worse than no row, because a red row gets
investigated and a green one does not. Two shipped examples, both green for
months:

* **`dispcp`** — a LIBRARY imported by 104 files, registered as a soak row. 22
  function definitions and no call: it booted nothing, asserted nothing, and
  reported `ok` in 0.1s against 60 declared.
* **`mkclick`** — a GENERATOR. It writes `build/click.mod`, a metronome module
  for judging A/V sync by eye and ear, and asserts nothing. Registered as a
  soak row declaring 10s, reporting `ok` in 0.0s.

The runner now says `UNDERRAN n% of its declared Ns` for a row that returns in
under 5% of its declaration, which catches the crudest version. It does not
catch a row that boots a machine, drives a session and then compares something
...
The corollary is the positive control. `lzfence` is the model: four hostile
streams that must be REFUSED, and a decoder that refuses everything passes all
four — so a valid stream runs first and its twelve bytes are compared one by
one.
...
A `full` row earns its place with **breadth per second**: `bootsmoke` is eight
seconds and fails for almost any serious regression, wherever it was. A row
that can only fail for one narrow reason belongs in `soak`, next to the change
that would break it. Ten minutes is about eight emulator rows, not fifty.
...
Row("lzmod", "soak", py("tests/lzmod.py"), 30.0,
    ...
    "All 116,085 bytes are compared BYTE FOR BYTE, because a decoder that "
    "got one match wrong across the boundary still opens a window, still "
...
| field | rule |
|---|---|
| `name` | what `-k` matches. Short, and the same as the script where there is one. |
| `tier` | §2. |
...
| `needs` | capabilities, PROBED not configured: `marty`, `qemu`, `nasm`, `cc`, `wiredisk`. A missing one SKIPS the row, and a skip is the box declining to answer — never a pass. |
| `serial` | it drives an emulator. The runner keeps those in their own lane, `--marty-jobs` wide; the host-side rows fan out ahead of them. Forgetting it puts an emulator row in the host lane, where it competes with every other row on the box. |
...
it is skipped constantly.

The compression family arrived declaring **2,721 seconds** for fourteen rows
that take **701**: `lzship` 33.3s against 420 declared, `lzship-lzb` 31.7
against 600, `kzboot` 16.0 against 120, `lzmod-lzb` 21.7 against 300. Every one
was honest when written — they were declared against a shape where the row's
own cost was two full builds — and none was revisited when that stopped being
true.
...
* The runner reports `(declared Ns)` when a row exceeds **2× + 1s**, which is
  there to catch a row that got 3× slower, not to police a loaded box.
* It reports `UNDERRAN` below **5%**, which is there to catch the row that ran
  nothing.
* Between those two it says nothing, so the number is yours to keep honest.

Include a private tree's build if the row builds one — a cold tree is ~39s on
...
`builds=True` was 58 rows and 3.3 declared hours that no width helped. It is
**three** (`buildmatrix`, `ctoolchain`, `fdlgthumb`). Do not add a fourth
without reading this section and deciding it applies to you.

The hazard is not hypothetical. A `make` in the shared tree while other rows
read it is how the first full soak of this work lost nine rows to one
four-minute window, and a knob build landing in `build/` is reproducible in
about twenty seconds: three rows that pass 3/3 against a frozen tree pass
**0/3** against `build/` with a `make VIDEO=cga` loop running, all three dying
with *"the map describes a DIFFERENT kernel"*.
...
starts, when nothing else is running.
...
cached against an earlier kernel is the stale-scratch-disk trap; make's rules
already name those includes as prerequisites.
...
`tree()` builds into `build/trees/<knob>-<hash>/` under a per-tree `flock` held
across the build and no longer. Two rows with different knobs never meet; two
with the same knob share one tree and the second waits only for the first's
BUILD. `build/` is never written, so a person can `make` in the checkout while
a soak runs.
...
Four of the eight compression rows that shipped with `builds=True` needed
**nothing**: the shipped kernel and a fixture. `lzmod-lzb` built a whole kernel
under a knob to test the LZB decoder — and the shipped kernel carries both
decoders (SPEC.md 20.13.6), so the two arms came out byte-identical and the
only real difference was which format the FIXTURE was wrapped in.

Ask what actually differs before reaching for a tree.
...
handed, and `os88sym` has honoured `$OS88_BUILD` since it was written. With the
variable unset `at()` is the identity function, so an interactive run is
unchanged. Without it, under a frozen soak, a row asserts about one build's
fixture and boots another's.
...
with os88ui.boot("build/os8088-360.img", apps="build/apps360.img") as ui:
    w = ui.path("APPS/CALC.O88")          # drive, folder, package - all checked
    ui.menu_pick("Calc", "Close")
```

The verbs: `open_drive`, `open`, `path`, `window`, `wait_window`,
`raise_window`, `move_window`, `drag_window`, `close`, `uncover`,
`clear_desktop`, `listing`, `entry`, `scroll_to`, `menus`, `menu_pick`,
`toast`, `wait_toast`, `fs_of`, `settle`. `ui.m` is the Marty and `ui.mo` the
mouse, so dropping a layer needs no second connection.
...
Measured: the same three-step navigation is **0.35×** the host time and 0.35×
the guest cycles of the settle-and-hope spelling.
...
then compares is a black screen) and resolves an IBM machine name to its
GLaBIOS twin unless the row makes a case in `why_ibm`.
...
That is the mechanism behind the soak's central finding: twelve rows at width 1
and at width 3 with two extra CPU hogs were 1.06× slower and 12/12 passed in
both arms, while the same waits cost the guest up to **37% less work**.
**Contention does not make a row slow, it makes it LESS THOROUGH** — which is
why the wall times in every classification run showed nothing.

Four tools, in the order to reach for them:

| want | use |
|---|---|
| a specific thing to HAPPEN | `os88marty.until(m, cond, what, guest=N)` |
...
happening. Four of the last five soak failures were that one bug.
...
`stable` identical captures `quiet` apart is 2.0 HOST seconds before it can
return, and it cannot come down (a change arriving after one whole quiet round
happens 1 time in 19, so halving it would end one settle mid-repaint per 48).
...
instead of host ones — `dispcalc` went **376.3s → 197.2s**, 48%, with every
assertion still passing, because one line was 30% of all settle time and what
it was waiting for was thirteen bytes of a composition buffer.

`settle` remains right when the next line compares PIXELS. It is also
positively wrong for an operation that holds the gfx lock: the screen is *more*
still while such an operation is busy than when it is finished, so a settle
returns five seconds into a hard-disk install and the `with` block kills the
emulator mid-copy. `until` on the commit is the answer there.
...
and answers them correctly on `kern_small` too (`DRV_MAX` is 4 there).
...
* Host-side rows under `tests/unit/` use `harness.check(cond, what, why=…)` and
  `done()` instead, which does all of the above and adds `why=` — say what
  breaks, for the reader who finds it red in a year.
...
| # | what happened | rule |
|---|---|---|
| 1 | `dispcp`, a library, registered as a row: `ok` in 0.1s against 60 declared | §1 |
| 2 | `mkclick`, a generator, registered as a row: `ok` in 0.0s against 10 | §1 |
| 3 | Fourteen compression rows declaring 2,721s and taking 701 | §4 |
| 4 | One row's mid-run `make` opened a four-minute window that cost nine rows | §5 |
| 5 | Three rows, 0/3 against `build/` under a knob hammer, 3/3 against a tree | §5.2 |
| 6 | `launch(..., env=t.env)` — no such argument; three rows died in their first second | §5.2 |
| 7 | `blitcut` died inside `no_saver`, three frames below its own code, because only `OS88_BUILD` was set | §5.2 |
| 8 | Eleven images had a Makefile rule and no builder; rows died on `FileNotFoundError` frames from the cause | §5.1 |
| 9 | A stale `build/c64.bin` survived a cherry-pick; `c64part` failed with "not byte-identical", which reads as a broken package | §5.1 |
| 10 | Twelve rows under load: 1.06× slower, up to 37% less guest work done | §7 |
| 11 | Four of the last five soak failures were `for _ in range(N)` giving the guest fewer rounds under load | §7.1 |
| 12 | `lzdrv` waited on `DRVR_SEG`, which is written before the read, the expand, the bss and the attach | §7.2 |
| 13 | `settle` returning five seconds into a hard-disk install, because the gfx lock makes the screen *more* still while it works | §7.3 |
| 14 | `dispcalc` spending 30% of all settle time waiting for the screen when the subject was thirteen bytes | §7.3 |
| 15 | `lzdrv` reading DRVCALL's probe strings before the probe had run — green alone, red beside one other emulator | §8 |
| 16 | `lzship` walking `drv_tab` on a stride of 10 where the record is 16 | §8 |
| 17 | A row hardcoding an IBM ROM name, running on a box that had it and silently not on one that did not | §6 |
| 18 | A row whose screen saver came on during a wait, comparing a black screen | §6 |
...
| **docs/SOAK-PARALLEL.md** | the parallel runner, where the suite's time goes, and every measurement quoted above |
| **docs/HANDOFF-SOAK-FINDINGS.md** | worked diagnoses of rows that failed, and what was ruled out for each |
