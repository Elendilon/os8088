...
7. **The VMware absolute pointer** (SPEC.md §9.11). MartyPC has no backdoor
   ...
   **Since §9.11.7 that disk carries a different KERNEL as well as a
   different `SYSTEM.CFG`** — `kern_emu`, out of `build/emuk/`. The whole
   resident half is inside `%ifdef KERN_EMU` now and `VMMOUSE.DRV` is off the
   shipped disks entirely, so on the shipped kernel there is nothing left to
   turn on. `tests/vmmouse.py` sets `$OS88_BUILD` and `$OS88_DEFINES` so
   `os88sym` resolves against that build.
...
| **The VMware absolute pointer** (SPEC.md §9.11, §9.11.7 — `kern_emu`) | ➖ | ✅ | `make vmmousetest && python3 tests/vmmouse.py` (interactively: `make emu`, then `make run VMPORT=on` on that image) | QEMU's `pc` machine carries a `vmport` and a `vmmouse` by default, so the backdoor probe succeeds here exactly as it does under v86 in the browser — the one CI gate this browser-only feature gets. **Every recipe that DRIVES the pointer turns it off** (`-machine pc,vmport=off`): the Makefile's eight `$(QEMU)` sites, through a `$(QEMUMACH)` of its own so that a documented `QEMU=` override cannot swallow it, and `tests/ps2mouse.py`, `tests/heapmap.py` and `tests/vgadirty.py` in their own launch strings. `tools/mouse.py` drives `msserial`, and left on beside it the backdoor wins the contest and retires it — QEMU then splits coordinates from buttons across the two devices, `mouse_btn` sticks pressed, and the drag never ends. **The recipes that only BOOT a stock image neither turn it off nor need to** — the release zip's README, docs/LIVE-MEDIA.md, the modem recipe below — and since §9.11.7 the reason is stronger than "the bit is clear": the shipped kernel has no `vmm_row`, no `vmm_boot_x` and no poll site, and **no image this tree ships carries `VMMOUSE.DRV` at all**. There is nothing on those disks to read and no code that could name it. `build/emu.img` (`make emu`) is the one disk that has both, and it is also the only one that ships a `SYSTEM.CFG`. `tests/vmmouse.py` turns it back on with `-serial none`, asserts `mou_port` = 6, and injects absolute positions through `vmmouse` that must land on the pixel. MartyPC has no backdoor |
