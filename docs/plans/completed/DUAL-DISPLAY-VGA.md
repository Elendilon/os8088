...
docs/plans/completed/DUAL-DISPLAY-PLAN.md is the study that produced §39.12–§39.19 and the
...
3. **The whole extended-desktop machinery is built and is adapter-agnostic.**
   Steps 1–7 of docs/plans/completed/DUAL-DISPLAY-PLAN.md landed: the per-display context and
...
has its own handoff in docs/history/DUAL-DISPLAY-BUG2.md. The field machine can carry
...
8. ~~**A window cannot be dragged across the seam on a mixed-depth machine**~~
   ...
   **Not closed**: nobody has yet driven the original *manual* session across
   ...
   docs/history/DUAL-DISPLAY-BUG2.md rather than editing it.

   **→ docs/history/DUAL-DISPLAY-BUG2.md is the handoff**, and its box at the top now
...
9. **`tests/dispsave.py`'s VGA+Hercules leg flips its verdict on DEAD BYTES,
   ...
   **Item 8's `cur_dprev` verdict is still void**, for a reason that survives
   ...
   the merged tree — docs/history/DUAL-DISPLAY-BUG2.md §7 is where that is written
   down.
...
15. **"Switching Tracker to full screen then back corrupts the other
    ...
    **It is invisible to a round-trip test by construction**, which is why
    ...
    `fsx_surf` (slot 0x0311) hands the app the rect its bracket owns, in the
