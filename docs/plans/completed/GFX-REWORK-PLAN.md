...
5. **"Speed decides it."** It does not. docs/plans/completed/TEXT-PLAN.md §1.1 is the standing
...
- `kernel/band.inc` — the composer, **`make BAND=1`** (SPEC.md §5.9.6 — it was
  ...
  docs/plans/completed/TEXT-PLAN.md exists to get. What is left of its cost is the band clear
...
2. **`band_emit_x`, 18,776 cycles a bar.** That is `gfx_blit1_x`, whose 603
   ...
   build now — but `gfx_blit1` is API 0x032D as well, and every package that
