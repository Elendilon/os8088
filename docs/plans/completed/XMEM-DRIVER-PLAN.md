...
`osapi_snd_fm` (slot 0x00EA) stays resident, stamps the requesting instance
...
- `make kernsplit`: `kern_small` **byte-identical**. This touches `kern_big`
  only, and a `kern_small` size that moves is docs/history/KERN-SPLIT-PLAN.md §2's
...
`kern_big`, which is docs/history/KERN-SPLIT-PLAN.md §7's stated failure mode arriving
...
docs/history/KERN-SPLIT-PLAN.md §2's leak: this is shared code improved for both
