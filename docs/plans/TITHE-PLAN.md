...
| **3** | the round loop: plan, commit, **reveal**, combat with healing, spoils, HUD, log — with the **fully editable plan** (§5.0.2). **Hot-seat**, with §6.3.1's frozen opponent. **STARTED** (SPEC.md §97.12): every view the renderer draws is the engine's, a plan is re-applied to the frozen board on every edit, and plays, UNDO, COMMIT, the pass screen and a resolved round are BUILT; swaps, stances, orders, the removable plan list and the animated resolution follow | two humans play a whole match; neither learns anything about the other's plan before the reveal; any entry in a plan can be removed and the board is right afterwards |
