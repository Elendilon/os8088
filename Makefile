...
# NOSPINUP=1 takes SPEC.md 18.101's spin-up guard out, so a WRITE goes down the
# instant a READ has started the motor - the kernel before 18.101 and the arm
# docs/FIELD-NOTES.md 32's A/B is taken against. It is here for NOFDDPARK's
# reason one block up: the effect lives in the BIOS's own motor byte, and a
# gate that cannot turn the fix off cannot tell a guard that fired from a
# spindle that happened to be up to speed already. It is also the only thing
# keeping the unguarded ladder assembling. tests/dskspin.py is the row.
ifneq ($(NOSPINUP),)
VIDDEF += -DNO_SPINUP
endif
...
KNOBS := $(strip $(foreach k,VIDEO HERCSEG RTC DISKCNT DISKAL BOOTDIAG FLOPPY1 \
                             ...
                             KERN_SMALL KERN_EMU FSNOSTAMP THEMEDARK TITLESNAP SPLSTARS NOSIZESNAP NOFLUSHR NOUNAL BAND NOPLANE NOCOLFAST NOBLITCUT NOUIBLOCK NOMOUPRIV NOCHAINPRIV NOHEDGE NOCURDISK NOFDDPARK NOSPINUP VGADIRTY DLJUNK COMPRESS NOKZIP,\
                             $(if $($(k)),$(k)=$($(k)))))
...
$(BUILD)/ether360.img: $(BUILD)/boot360.bin $(KERNFILE) $(DRIVERS) $(SYSAPPS) $(COREAPPS) $(SYSDOC) $(SYSLOGO) $(FACES) $(FACELIC) $(BUILD)/system.cfg tools/os88disk.py
	python3 tools/os88disk.py -o $@ --size 360 \
		--boot $(BUILD)/boot360.bin --kernel $(KERNFILE) \
		$(DRIVERS) $(SYSAPPSARGS) $(COREAPPSARGS) $(SYSDOC) $(SYSLOGOARG) $(FACESARG) \
		$(BUILD)/system.cfg
...
$(IMG360): $(BUILD)/boot360.bin $(KERNFILE) $(DRIVERS) $(SYSAPPS) $(COREAPPS) $(SYSDOC) $(SYSLOGO) $(FACES) $(FACELIC) tools/os88disk.py
	python3 tools/os88disk.py -o $@ --size 360 \
		--boot $(BUILD)/boot360.bin --kernel $(KERNFILE) \
		$(DRIVERS) $(SYSAPPSARGS) $(COREAPPSARGS) $(SYSDOC) $(SYSLOGOARG) $(FACESARG) \
		$(APPDATAFOLDER)
