...
is `docs/plans/completed/SOAK-PARALLEL.md` 1: twelve rows at width 3 with two extra CPU hogs
...
def _default_mj():
    ...
    docs/plans/completed/SOAK-PARALLEL.md 1: twelve rows at width 3 with two extra CPU hogs
...
def prebuild(rows):
    ...
    **THE POINT IS THE MOMENT, not the convenience.** A row that builds its own
    ...
    (docs/plans/completed/SOAK-PARALLEL.md 12). Doing it here does it when nothing else is
    running.
...
def main():
    ...
    def report(res):
        if res.skipped:
            ...
            # fast one. docs/plans/completed/HANDOFF-SOAK-FINDINGS.md B4 records three rows
