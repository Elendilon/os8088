...
def emit(path, binpath=None):
    """THE BASES ARE A PART (SPEC.md 97.8, TITHE-PLAN 4.3): `binpath` gets the
    table and every record, laid out at offset 0 of the part, and `path` gets
    only what the package itself needs - the counts and the names. They were
    10.8 KB of the package's own 60 KB until wave 3 left it 1.2 KB of room.

    The part's layout: TI_BART_N x TI_BART_G words, each the offset of a
    record; a record is nine words (the ground, then the eight moves) and then
    their bytes - a ground is (w/8) x h packed, a move `db x, y, w, h` and its
    (w/8) x h bytes. Every offset is from the part's own start."""
    ...
    for ci, cand in enumerate(cands):
        for gi, (gname, w, h, _) in enumerate(SURFACES):
            ...
            for p in range(POSES):
                ...
                bx = bbox(bp, g2)
                if bx is None:
                    moves.append(bytes(4))
                    continue
                x0, y0, ww, hh = bx
                pb = pack(bp, x0, y0, ww, hh)
                total += len(pb)
                moves.append(bytes([x0, y0, ww, hh]) + pb)
            recs.append((gb, moves))
    blob = bytearray(2 * len(recs))
    for i, (gb, moves) in enumerate(recs):
        at = len(blob)
        _st.pack_into("<H", blob, 2 * i, at)
        hdr = 2 * (1 + POSES)
        offs, cur = [], at + hdr
        for piece in [gb] + moves:
            offs.append(cur)
            cur += len(piece)
        blob += _st.pack("<%dH" % len(offs), *offs)
        for piece in [gb] + moves:
            blob += piece
    if len(blob) > 0xFFFF:
        print("%s: %d bytes is past one segment" % (path, len(blob)))
        return 0
    lines = ["; GENERATED by tools/os88tithebase.py - do not edit.",
             "; TITHE's base art (SPEC.md 97.2.1, 97.5.1): a shared GROUND and",
             "; a small moving sub-band per pose, one set per surface. The art",
             "; itself is a PART (build/tibase.bin, TI_BASE_PART in tithe.asm):",
             "; this file carries only what the package needs to find it.",
             "",
             "; in this build: " + ", ".join(c[0] for c in cands),
             "TI_BART_N   equ %d" % len(cands),
             "TI_BART_G   equ %d" % len(SURFACES),
             "TI_BASE_BYTES equ %d          ; the part, unpacked" % len(blob),
             ""]
    ...
    print("%s + %s: %d of %d candidates x %d surfaces, %d bytes of art, a "
          "%d-byte part" % (path, binpath, len(cands), len(CANDIDATES),
                            len(SURFACES), total, len(blob)))
    return len(blob)
...
def main():
    ...
    ap.add_argument("--bin", default="build/tibase.bin",
                    help="emit: where the PART goes")
    a = ap.parse_args()
    if a.selfcheck:
        return selfcheck()
    ...
    if a.cmd == "sizes":
        ...
        return 0
    if a.cmd == "emit":
        return 0 if emit("apps/tithe/tibases.inc", a.bin) else 1
    if a.cmd == "insitu":
