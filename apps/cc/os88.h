 ...
 *
 * The count: 101 of the 160 slots apps/os88api.inc publishes, plus six
 * window-record accessors and six runtime helpers that are not slots at all -
 * 129 C entry points, and every one of them is in apps/cc/os88thunk.asm.
 *
 ...
 *   101  those names that appear in apps/cc/os88thunk.asm, in code rather
 *        than in a comment - a macro invocation names its slot as an
 *        argument, so a plain grep finds them all.
 *   129  distinct `_os88_*` entry points defined in that same file, whether
 *        written out or generated by one of the CC_T_* macros. It is not
 *        101 + 12: some slots have two entry points (the _seg forms), and
