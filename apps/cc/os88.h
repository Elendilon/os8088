 ...
 *
 * The count: 97 of the 155 slots apps/os88api.inc publishes, plus the
 * window-record accessors and the runtime helpers that are not slots at all -
 * 125 C entry points, and every one of them is in apps/cc/os88thunk.asm.
 *
 ...
 *    97  those names that appear in apps/cc/os88thunk.asm, in code rather
 *        than in a comment - a macro invocation names its slot as an
 *        argument, so a plain grep finds them all.
 *   125  distinct `_os88_*` entry points defined in that same file, whether
 *        written out or generated by one of the CC_T_* macros. It is not
 *        97 + 12: some slots have two entry points (the _seg forms), and
