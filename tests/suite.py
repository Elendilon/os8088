...
SOAK = [
    ...
    Row("pkgrun", "soak", py("tests/pkgrun.py"), 25.0,
        ...
        needs=("marty",),
        wants=("build/pkgrun360.img", "build/hello.o88", "build/mseg.o88")),
    ...
    Row("dirwshed", "soak", py("tests/dirwshed.py"), 45.0,
        ...
        "consumer: the Setup page shows both figures and a check box picks "
        ...
        "and [dsk_rah_seg] - the KERNEL's own word - is 0 afterwards, without "
        "which the row would pass on a box that asked for the bigger number "
        "and was quietly given the smaller. IT READS STATE AND NOT THE GLASS. "
        "THE ORDER IS LOAD-BEARING, twice: the page is read BEFORE any launch "
        "because the keepc=0 launch SHEDS the cache and a page read after it "
        "correctly reports two equal numbers about a machine that no longer "
        "has one; and the launches happen with the CONSOLE up, because the "
