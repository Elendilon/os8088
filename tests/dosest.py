    ...
    with os88ui.boot(SYS, apps=APPS, machine=a.machine) as ui:
        ...
        if msnk != held:
            fail("the page's sound term is %d K and the class is HOLDING %d. "
                 ...
                 "drv_memk - DRVM_SND is 7 image + 8 DMA + 20 SBL_POOLKB, and "
                 "the pool is claimed on the FIRST GRANT and the buffer per "
                 "stream, so a mounted silent driver holds its 7 and the "
                 "constant is 28 high for ever (SPEC.md 51.12.2, 34.5.2)"
                 % (msnk, held))
