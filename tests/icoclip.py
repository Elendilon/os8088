    ...
    with os88marty.launch(os.path.join(ROOT, "build", "os8088-360.img"),
        ...
        if not mono:
            raise SystemExit("icoclip: %s came up NOT mono (vid_mono=%d) - "
                             ...
                             % (tag, mono))
        c = Caller(m)
        rec = os88sym.linear("ico_disk32") - KB
        hdr = m.read(os88sym.linear("ico_disk32"), 2)
        ww, ih = hdr[0], hdr[1]
        if (ww, ih) != (2, 32):
            raise SystemExit("icoclip: ico_disk32 is %dx%d words/rows, not the "
                             "32-wide record this row is written for"
                             % (ww, ih))
        ...
        def draw(x):
            clear()
            c.call("icon_draw", cx=x, dx=ICON_Y, si=rec)
        ...
        for phase in range(8):
            ...
            if dump is not None:
                dump["%s/%d/ref" % (tag, phase)] = b"".join(ref).hex()
            ...
            for col0 in range(stride - span + 1, stride):
                ...
                if dump is not None:
                    dump["%s/%d/%d" % (tag, phase, col0)] = b"".join(got).hex()
        ...
        print("%s: stride %d, 8 phases x %d clipped columns, %d findings"
              % (tag, stride, span - 1, len(bad)))
    return bad
