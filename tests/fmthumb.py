...
"""SPEC.md 13.10.5: the Disk window's scroll-bar THUMB is dragged.
    ...
    make SBDRAG=1 && python3 tests/fmthumb.py [machine]

A KNOB build's gate, so it is not in tests/suite.py's tiers - `make SBDRAG=1`
first or every case below fails on a kernel that has no such feature.
...
with M.launch("build/os8088-360.img", apps=DISK,
    ...
    before = lit(m, mono, band)
    target = y2 - SBCELL - 1 - h // 2        # the bottom of the track
